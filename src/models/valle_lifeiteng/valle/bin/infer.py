...
def get_args():
    ...
    parser.add_argument(
        "--text-tokens",
        type=str,
        default="src/models/valleiteng/egs/libritts/data/tokenized/unique_text_tokens.k2symbols",
...
def main():
    ...
    for n, text in enumerate(args.text.split("|")):
        ...
        text_tokens, text_tokens_lens = text_collater(
            [
                tokenize_text(
                    text_tokenizer, text=f"{text}".strip() if text_prompts == "" \
                        else f"{text_prompts} {text}".strip()
                )
            ]
        )
