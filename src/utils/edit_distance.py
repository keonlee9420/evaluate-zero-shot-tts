...
Migrated from speechbrain.utils.edit_distance.
Due to dependency issues, we cannot import the original module.
...
def wer_details_by_utterance(
    ...
    for key, ref_tokens in ref_dict.items():
        ...
        if len(ref_tokens) == 0 or len(hyp_tokens) == 0:
            continue  # Skip scoring this utterance
        elif ref_tokens[0] == "" and hyp_tokens[0] == "":
            num_ref_tokens = 0
        else:
            num_ref_tokens = len(ref_tokens)
...
def print_wer_and_alignment(ids, refs, hyps, file=sys.stdout):
    details = wer_details_by_utterance(ids, refs, hyps)
    summary = wer_summary(details)
    print_wer_summary(summary, file=file)
    print_alignments(details, file=file)
