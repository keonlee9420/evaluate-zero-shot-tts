...
class TestElectronic:
    ...
    @parameterized.expand(
        parse_test_case_file(
            "en/data_inverse_text_normalization/test_cases_electronic.txt"
        )
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_denorm(self, test_input, expected):
        pred = self.inverse_normalizer_en.inverse_normalize(
            test_input, verbose=False
        )
        assert pred == expected
        pred = self.inverse_normalizer_en_cased.inverse_normalize(
            test_input, verbose=False
        )
        assert pred == expected
    ...
    @parameterized.expand(
        parse_test_case_file(
            "en/data_inverse_text_normalization/test_cases_electronic_cased.txt"
        )
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    ...
    normalizer_en = Normalizer(
        input_case="cased", cache_dir=CACHE_DIR, overwrite_cache=False
    )
    ...
    @parameterized.expand(
        parse_test_case_file(
            "en/data_text_normalization/test_cases_electronic.txt"
        )
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
