...
class TestCardinal:
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ar/data_inverse_text_normalization/test_cases_cardinal.txt"
        )
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_denorm(self, test_input, expected):
        pred = self.inverse_normalizer_ar.inverse_normalize(
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ar/data_text_normalization/test_cases_cardinal.txt"
        )
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_norm(self, test_input, expected):
        pred = self.normalizer_ar.normalize(
