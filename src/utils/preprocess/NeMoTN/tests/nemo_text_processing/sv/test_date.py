...
class TestDate:
    inverse_normalizer_sv = InverseNormalizer(
        lang="sv", cache_dir=CACHE_DIR, overwrite_cache=False
    )
    ...
    @parameterized.expand(
        parse_test_case_file(
            "sv/data_inverse_text_normalization/test_cases_date.txt"
        )
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_denorm(self, test_input, expected):
        pred = self.inverse_normalizer_sv.inverse_normalize(
            test_input, verbose=False
        )
        assert pred == expected

        pred = self.inverse_normalizer_sv_cased.inverse_normalize(
            test_input, verbose=False
        )
        assert pred == expected

    normalizer_sv = Normalizer(
        input_case="cased",
        lang="sv",
        cache_dir=CACHE_DIR,
        overwrite_cache=False,
    ...
    @parameterized.expand(
        parse_test_case_file("sv/data_text_normalization/test_cases_date.txt")
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
