...
class OrdinalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        graph_firsts = pynini.string_file(
            get_abs_path("data/ordinals/firsts.tsv")
        )
        graph_second = pynini.string_file(
            get_abs_path("data/ordinals/second.tsv")
        )
        graph_special_ordinals = pynini.string_file(
            get_abs_path("data/ordinals/key_nouns.tsv")
        )

        # Removes morpheme
        graph_no_root_change = pynutil.delete("ième")  # For no change to root

        graph_strip_morpheme = pynini.union(
            graph_no_root_change, graph_undo_root_change
        )
        graph_strip_morpheme = NEMO_SIGMA + graph_strip_morpheme

        graph_integer_component = graph_strip_morpheme @ graph_cardinal

        graph_morpheme_component = pynutil.insert(
            "e"
        )  # Put the superscript in.
        graph_morpheme_component += pynini.accep(
            "s"
        ).ques  # In case of plurals.
        ...
        # 'premier' or 'première'
        graph_firsts = (
            pynutil.insert('integer: "1" morphosyntactic_features: "')
            + graph_firsts
        )

        # Second used 'd' as a superscript.
        graph_second = (
            pynutil.insert('integer: "2" morphosyntactic_features: "')
            + graph_second
        )
        ...
        graph_special_ordinals = (
            pynutil.insert("/") + delete_space + graph_special_ordinals
        )
