...
class TimeFst(GraphFst):
    ...
    def __init__(self):
        ...
        graph_suffix_am = pynini.string_file(
            get_abs_path("data/time/time_suffix_am.tsv")
        )
        graph_suffix_pm = pynini.string_file(
            get_abs_path("data/time/time_suffix_pm.tsv")
        )

        graph_suffix = pynini.cross(graph_suffix_am, "am") | pynini.cross(
            graph_suffix_pm, "pm"
        )
        ...
        graph_demi += pynini.accep(
            "e"
        ).ques  # people vary on feminine or masculine form
        ...
        graph_hours_component = pynini.union(
            graph_hours, graph_midi, graph_minuit
        )
        ...
        graph_time_standard = (
            graph_hours_component + graph_minutes_component.ques
        )
        ...
        graph_minutes_to_component = pynini.union(
            graph_minutes, graph_fractions
        )
        ...
        graph_time_to = (
            graph_hours_to_component + graph_moins + graph_minutes_to_component
        )

        graph_time_no_suffix = graph_time_standard | graph_time_to

        graph_suffix_component = (
            pynutil.insert(' suffix: "') + graph_suffix + pynutil.insert('"')
        )
