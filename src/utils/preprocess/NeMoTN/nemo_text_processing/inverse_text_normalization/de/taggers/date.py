...
class DateFst(GraphFst):
    ...
    def __init__(
        ...
        optional_delete_space = pynini.closure(
            NEMO_SIGMA | pynutil.delete(" ", weight=0.0001)
        )
        ...
        month_as_string = (
            pynutil.delete('month: "')
            + tn_date_tagger.month_abbr.invert()
            + pynutil.delete('"')
        )

        convert_year = (
            (tn_date_tagger.year @ optional_delete_space).invert().optimize()
        )
        delete_year_marker = (
            pynutil.delete('year: "')
            + pynini.closure(NEMO_NOT_QUOTE, 1)
        ...
        verbalizer = (
            pynini.closure(
                delete_day_marker + pynutil.insert(".") + pynini.accep(" "),
                0,
                1,
            )
            + month_as_string
