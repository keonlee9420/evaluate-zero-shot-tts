...
import pynini
from nemo_text_processing.inverse_text_normalization.sv.utils import (
    get_abs_path,
)
...
from nemo_text_processing.text_normalization.sv.utils import (
    get_abs_path as get_tn_abs_path,
)
...
def get_all_to_or_from_fst(cardinal: GraphFst):
    ...
    for key in numbers:
        ...
        for when in ["över", "i"]:
            ...
            output[key][when] = (
                pynini.project(map, "input") @ map @ cardinal.graph
            )
    return output
...
class TimeFst(GraphFst):
    ...
    def __init__(self, tn_cardinal_tagger: GraphFst):
        ...
        suffixes = pynini.invert(
            pynini.string_map(load_labels(get_abs_path("data/time/suffix.tsv")))
        )
        self.suffixes = suffixes

        klockan = pynini.union(
            pynini.cross("klockan", "kl."), pynini.cross("klockan är", "kl.")
        )
        klockan_graph_piece = pynutil.insert('hours: "') + klockan
        minutes_to = pynini.string_map(
            [(str(i), str(60 - i)) for i in range(1, 60)]
        )
        minutes = pynini.string_map([str(i) for i in range(1, 60)])
        minutes_inverse = pynini.invert(
            pynini.project(minutes_to, "input") @ tn_cardinal_tagger.graph_en
        )
        minutes = pynini.invert(
            pynini.project(minutes, "input") @ tn_cardinal_tagger.graph_en
        )
        self.minute_words_to_words = (
            minutes_inverse @ minutes_to @ tn_cardinal_tagger.graph_en
        )
        ...
        time_zone_graph = pynini.invert(
            pynini.string_file(get_tn_abs_path("data/time/time_zone.tsv"))
        )
        final_suffix = (
            pynutil.insert('suffix: "') + suffixes + pynutil.insert('"')
        )
        final_suffix_optional = pynini.closure(NEMO_SPACE + final_suffix, 0, 1)
        final_time_zone = (
            pynutil.insert('zone: "') + time_zone_graph + pynutil.insert('"')
        )
        final_time_zone_optional = pynini.closure(
            NEMO_SPACE + final_time_zone, 0, 1
        )
        both_optional_suffixes = (
            final_suffix_optional + final_time_zone_optional
        )
        one_optional_suffix = (
            NEMO_SPACE + final_suffix + final_time_zone_optional
        )
        one_optional_suffix |= (
            final_suffix_optional + NEMO_SPACE + final_time_zone
        )

        labels_hour = [str(x) for x in range(0, 24)]
        hours = pynini.invert(
            pynini.union(*labels_hour) @ tn_cardinal_tagger.graph
        )
        self.hours = hours
        hours_graph = pynutil.insert('hours: "') + hours + pynutil.insert('"')
        klockan_hour = (
            klockan_graph_piece + NEMO_SPACE + hours + pynutil.insert('"')
        )
        ...
        hours_to = pynini.string_map(
            [(str(x[0]), str(x[1])) for x in hours_to_pairs()]
        )
        hours_to = pynini.invert(hours_to @ tn_cardinal_tagger.graph)
        self.hours_to = hours_to
        hours_to_graph = (
            pynutil.insert('hours: "') + hours_to + pynutil.insert('"')
        )

        bare_quarters_to = pynini.string_map(
            [(x[1], str(x[0])) for x in QUARTERS.items() if not "över" in x[1]]
        )
        ...
        prefix_minutes_to_graph = (
            pynutil.insert('minutes: "')
            + prefix_minutes_to
            + pynutil.insert('"')
        )
        graph_to_prefixed = (
            prefix_minutes_to_graph + NEMO_SPACE + hours_to_graph
        )
        prefix_minutes_from_graph = (
            pynutil.insert('minutes: "')
            + prefix_minutes_from
            + pynutil.insert('"')
        )
        graph_from_prefixed = (
            prefix_minutes_from_graph + NEMO_SPACE + hours_graph
        )
        minutes_graph = (
            pynutil.insert('minutes: "') + minutes + pynutil.insert('"')
        )
        seconds_graph = (
            pynutil.insert('seconds: "') + minutes + pynutil.insert('"')
        )
        ...
        graph = (
            graph_to_prefixed
            | graph_from_prefixed
            | klockan_hour + both_optional_suffixes
            | hour_sfx
        )
        graph |= hm_sfx
        graph |= hms_sfx
