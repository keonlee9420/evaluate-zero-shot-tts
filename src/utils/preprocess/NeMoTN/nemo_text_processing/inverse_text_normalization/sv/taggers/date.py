...
class DateFst(GraphFst):
    ...
    def __init__(
        ...
        month_double = (
            force_double_digits(tn_date_tagger.number_to_month)
            .invert()
            .optimize()
        )
        month_abbr = tn_date_tagger.month_abbr.invert().optimize()
        self.month_to_number = (
            tn_date_tagger.number_to_month.invert().optimize()
        )

        graph_year = pynutil.insert('year: "') + year + pynutil.insert('"')
        graph_month = (
            pynutil.insert('month: "') + month_double + pynutil.insert('"')
        )
        graph_month_abbr = (
            pynutil.insert('month: "') + month_abbr + pynutil.insert('"')
        )
