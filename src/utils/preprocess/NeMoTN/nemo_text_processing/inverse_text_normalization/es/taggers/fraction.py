...
class FractionFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, ordinal: GraphFst):
        ...
        numbers_read_as_ordinals = pynini.string_file(
            get_abs_path("data/fractions/numbers_read_as_ordinals.tsv")
        )
        ordinal_replacements = pynini.string_file(
        ...
        ordinal_exceptions = pynini.project(
            pynini.union("segundo", "tercero", "cuarto"), "input"
        )

        ordinal_graph |= ordinal_replacements @ ordinal_graph
        ordinal_graph = (
            pynini.project(ordinal_graph, "input")
            - ordinal_exceptions.arcsort()
        ) @ ordinal_graph
        ...
        restore_accents = pynini.string_map(
            [("un", "ún"), ("dos", "dós"), ("tres", "trés")]
        )
        restore_accents = NEMO_SIGMA + pynini.closure(
            pynutil.add_weight(restore_accents, -0.001), 0, 1
        )
        extend_numbers = (
            NEMO_SIGMA
            + pynini.closure(pynini.cross("i", "a y "), 0, 1)
            + NEMO_SIGMA
        )
        delete_endings = (
            NEMO_SIGMA
            + (pynutil.delete("avo") | pynutil.delete("vo"))
            + make_singular
        )
        other_denominators = (
            extend_numbers @ delete_endings @ restore_accents @ cardinal_graph
        )
        ...
        optional_negative_graph = pynini.closure(
            pynini.cross("menos", 'negative: "True"') + NEMO_SPACE, 0, 1
        )
        ...
        optional_integer_part_graph = pynini.closure(
            integer_part_graph + NEMO_SPACE, 0, 1
        )
        ...
        proper_fractions_with_medio = proper_fractions | (
            pynutil.insert('numerator: "1" ')
            + pynini.cross("medio", 'denominator: "2"')
        )
        proper_fractions_with_medio = (
            optional_negative_graph + proper_fractions_with_medio
        )

        self.proper_fractions_with_medio = self.add_tokens(
            proper_fractions_with_medio
        )

        graph = (
            optional_negative_graph
            + optional_integer_part_graph
            + numerators_graph
            + NEMO_SPACE
            + denominators_graph
        )
