...
class DateFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        graph_suffix = pynini.string_file(
            get_abs_path("data/dates/year_suffix.tsv")
        ).invert()
        ...
        graph_1_to_31 = pynini.union(
            graph_1_to_31, pynini.cross("primero", "1")
        )

        day_graph = (
            pynutil.insert('day: "') + graph_1_to_31 + pynutil.insert('"')
        )

        month_graph = (
            pynutil.insert('month: "') + graph_month + pynutil.insert('"')
        )
        ...
        roman_centuries_graph = (
            pynutil.insert('year: "') + roman_centuries + pynutil.insert('"')
        )

        # transform "doscientos antes de cristo" -> "200 a. c."
        year_with_suffix = cardinal.graph + pynini.accep(" ") + graph_suffix
        year_with_suffix_graph = (
            pynutil.insert('year: "') + year_with_suffix + pynutil.insert('"')
        )
