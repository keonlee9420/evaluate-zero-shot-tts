...
def get_quantity(decimal, cardinal):
    ...
    res = res | decimal + pynutil.insert(
        ' quantity: "'
    ) + suffix + pynutil.insert('"')
...
class DecimalFst(GraphFst):
    def __init__(self, cardinal: GraphFst):
        ...
        cardinal_after_decimal = pynini.string_file(
            get_abs_path("data/numbers/digit-nano.tsv")
        ) | pynini.closure(pynini.cross("零", "0"))
        cardinal_before_decimal = cardinal.just_cardinals | pynini.cross(
            "零", "0"
        )
        ...
        graph_decimal_no_sign = pynini.closure(
            (graph_integer + delete_decimal + graph_fractional), 1
        )
        ...
        graph_negative = pynini.cross("负", 'negative: "-" ') | pynini.cross(
            "負", 'negative: "-" '
        )
        graph_negative = pynini.closure(
            graph_negative, 0, 1
        )  # captures only one "负"

        graph_decimal = graph_negative + graph_decimal_no_sign
        graph_decimal = graph_decimal | (
            graph_negative
            + get_quantity(graph_decimal_no_sign, cardinal_before_decimal)
        )
