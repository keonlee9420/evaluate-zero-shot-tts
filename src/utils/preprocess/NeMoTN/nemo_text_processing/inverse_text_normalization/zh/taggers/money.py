...
class MoneyFst(GraphFst):
    def __init__(self, cardinal: GraphFst, decimal: GraphFst):
        ...
        graph_fractional_values = (
            graph_cardinal @ add_leading_zero_to_double_digit
        )  #
        ...
        # regular symbol part
        graph_major_currency = (
            pynutil.insert('currency: "') + major_currency + pynutil.insert('"')
        )
        graph_minor_currency = (
            pynutil.insert('currency: "') + minor_currency + pynutil.insert('"')
        )

        # regular combine number and symbol part
        graph_only_major = (
            graph_integer_component + pynutil.insert(" ") + graph_major_currency
        )
        graph_only_minor = (
            graph_fractional_component
            + pynutil.insert(" ")
        ...
        graph_money = (
            graph_only_major + pynutil.insert(" ") + graph_fractional_component
        )

        # regular large money with decimals
        graph_large_money = (
            graph_decimal + pynutil.insert(" ") + graph_major_currency
        )
        ...
        graph_digits = (
            pynutil.insert('fractional_part: "') + digits + pynutil.insert('"')
        )
        graph_kuaimao = (
            graph_kuai
            + pynutil.insert(" ")
