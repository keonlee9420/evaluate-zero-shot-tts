...
class DateFst(GraphFst):
    def __init__(self):
        ...
        )  # imported for year-component
        months = pynini.string_file(
            get_abs_path("data/date/months.tsv")
        )  # imported for month-component
        days = pynini.string_file(
            get_abs_path("data/date/day.tsv")
        )  # imported for day-component
        ...
        graph_year = (
            pynutil.insert('year: "') + graph_year + pynutil.insert('"')
        )
        ...
        # grammar for day
        graph_day_suffix = (
            pynini.accep("日") | pynini.accep("号") | pynini.accep("號")
        )
        graph_delete_day_suffix = pynutil.delete(graph_day_suffix)
        graph_day = (
            pynutil.insert('day: "')
            + days
            + graph_delete_day_suffix
            + pynutil.insert('"')
        )
        ...
        graph_bc_prefix = (
            pynini.closure("紀元前", 0, 1)
            | pynini.closure("公元前", 0, 1)
            | pynini.closure("纪元前", 0, 1)
        )
