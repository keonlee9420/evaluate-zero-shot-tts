...
class CardinalFst(GraphFst):
    ...
    def __init__(self, use_strict_e=False):
        ...
        graph_one_hundred = pynini.string_file(
            get_abs_path("data/numbers/onehundred.tsv")
        )
        ...
        if not use_strict_e:
            ...
            graph_hundred_component = pynini.union(
                graph_hundred_component, graph_one_hundred
            )
            ...
            graph = (
                pynini.cdrewrite(
                    pynutil.delete("e"), NEMO_SPACE, NEMO_SPACE, NEMO_SIGMA
                )
            ...
            graph_e = (
                pynutil.delete(NEMO_WHITE_SPACE.plus)
                + pynutil.delete("e")
                + pynutil.delete(NEMO_WHITE_SPACE.plus)
            )
            ...
            graph_hundred_component_prefix_e = pynini.union(
                graph_one_hundred,
                pynutil.add_weight(
                    graph_hundreds_except_hundred + pynutil.insert("00"), 0.1
                ),
            ...
            graph_hundred_component_prefix_e = (
                graph_hundred_component_prefix_e.optimize()
            )
            ...
            graph_hundred_component_no_prefix = (
                graph_hundred_component_no_prefix.optimize()
            )
            ...
            graph_milhao_prefix_e = pynini.union(
                ...
                + (
                    (graph_e + graph_mil_prefix_e)
                    | (delete_space + graph_mil_no_prefix)
                )
            )
            ...
            graph_bilhao_prefix_e = pynini.union(
                ...
                + (
                    (graph_e + graph_milhao_prefix_e)
                    | (delete_space + graph_milhao_no_prefix)
                )
            )
            ...
            graph_trilhao_prefix_e = pynini.union(
                ...
                + (
                    (graph_e + graph_bilhao_prefix_e)
                    | (delete_space + graph_bilhao_no_prefix)
                )
            )
            ...
            graph_trilhao_no_prefix = pynini.union(
                (
                    (
                        ...
                        + delete_space
                        + (
                            pynutil.delete("trilhão")
                            | pynutil.delete("trilhões")
                        )
                    )
                    | pynutil.insert("000", weight=0.1)
                )
            ...
            graph_quatrilhao_prefix_e = pynini.union(
                (
                    ...
                    + delete_space
                    + (
                        pynutil.delete("quatrilhão")
                        | pynutil.delete("quatrilhões")
                    )
                )
                + (
                    (graph_e + graph_trilhao_prefix_e)
                    | (delete_space + graph_trilhao_no_prefix)
                )
            )
            ...
            graph_quintilhao_prefix_e = pynini.union(
                (
                    ...
                    + delete_space
                    + (
                        pynutil.delete("quintilhão")
                        | pynutil.delete("quintilhões")
                    )
                )
                + (
            ...
            graph_sextilhao_prefix_e = pynini.union(
                (
                    ...
                    + delete_space
                    + (
                        pynutil.delete("sextilhão")
                        | pynutil.delete("sextilhões")
                    )
                )
                + (
        ...
        digits_up_to_thousand = (
            NEMO_DIGIT | (NEMO_DIGIT**2) | (NEMO_DIGIT**3)
        )
        numbers_up_to_thousand = pynini.compose(
            graph, digits_up_to_thousand
        ).optimize()
        ...
        digits_from_year = (
            numbers_up_to_million @ pynini.union(*digits_1_2099)
        ).optimize()
        self.digits_from_year = digits_from_year

        # don't convert cardinals from zero to nine inclusive
        graph_exception = pynini.project(
            pynini.union(graph_digit, graph_zero), "input"
        )
