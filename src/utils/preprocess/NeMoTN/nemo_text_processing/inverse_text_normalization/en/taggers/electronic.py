...
class ElectronicFst(GraphFst):
    ...
    def __init__(self, input_case: str = INPUT_LOWER_CASED):
        ...
        url_symbols = pynini.string_file(
            get_abs_path("data/electronic/url_symbols.tsv")
        ).invert()
        ...
        single_alphanum = (
            pynini.closure(alpha_num + delete_extra_space) + alpha_num
        )
        server = (
            single_alphanum
            | pynini.string_file(
        ...
        if input_case == INPUT_CASED:
            protocol_end = pynini.cross(
                pynini.union(*get_various_formats("www")), "www"
            )

            protocol_start = pynini.cross(
                pynini.union(*get_various_formats("http")), "http"
            ) | pynini.cross(
        ...
        protocol |= (
            pynini.closure(
                protocol_end + delete_extra_space + process_dot, 0, 1
            )
            + protocol_default
        )
        ...
        if input_case == INPUT_CASED:
            graph = capitalized_input_graph(
                graph, capitalized_graph_weight=MIN_POS_WEIGHT
            )
