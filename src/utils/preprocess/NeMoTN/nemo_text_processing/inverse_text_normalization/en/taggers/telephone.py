...
def get_serial_number(cardinal):
    ...
    two_digit = pynutil.add_weight(
        pynini.compose(cardinal.graph_two_digit, NEMO_DIGIT**2), 0.002
    )
    character = digit | two_digit | NEMO_ALPHA
    sequence = (NEMO_LOWER_NOT_A | digit) + pynini.closure(
        pynutil.delete(" ") + character, 2
    )
    sequence |= character + pynini.closure(
        pynutil.delete(" ") + (digit | NEMO_ALPHA), 2
    )
    sequence2 = (
        NEMO_ALPHA
    ...
    sequence2 |= NEMO_LOWER_NOT_A + pynini.closure(
        pynutil.delete(" ") + two_digit, 1
    )
    sequence2 |= (
        two_digit
        + pynini.closure(pynutil.delete(" ") + two_digit, 1)
    ...
    sequence = (sequence | sequence2) @ (
        pynini.closure(NEMO_ALNUM) + NEMO_DIGIT + pynini.closure(NEMO_ALNUM)
    )
...
class TelephoneFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, input_case: str = INPUT_LOWER_CASED):
        ...
        single_double_or_triple_digit = (
            pynutil.add_weight(triple_digit_to_digit, -0.0001)
            | single_or_double_digit
            | delete_space
        ).optimize()
        ...
        single_or_double_digit |= (
            ...
            + pynini.closure(
                pynutil.add_weight(
                    pynutil.delete(" ") + single_or_double_digit, 0.0001
                )
            )
        ).optimize()
        single_double_or_triple_digit |= (
            ...
            + pynini.closure(
                pynutil.add_weight(
                    pynutil.delete(" ") + single_double_or_triple_digit, 0.0001
                )
            )
        ).optimize()
        ...
        cardinal_option = pynini.compose(
            single_double_or_triple_digit, NEMO_DIGIT ** (2, 3)
        )
        ...
        credit_card_graph = pynini.compose(
            single_double_or_triple_digit,
            NEMO_DIGIT**4
            + (space_six_digits | (space_four_digits**2))
            + space_four_digits,
        ).optimize()
        ...
        # ip
        digit_or_double = (
            pynini.closure(str_to_digit + pynutil.delete(" "), 0, 1)
            + double_digit_to_digit
        )
        digit_or_double |= double_digit_to_digit + pynini.closure(
            pynutil.delete(" ") + str_to_digit, 0, 1
        )
        digit_or_double |= str_to_digit + (
            pynutil.delete(" ") + str_to_digit
        ) ** (0, 2)
        ...
        ip_graph = (
            digit_or_double
            + (pynini.cross(" dot ", ".") + digit_or_double) ** 3
        )
        ...
        graph |= (
            pynutil.insert('number_part: "')
            + pynutil.add_weight(
                get_serial_number(cardinal=cardinal), weight=0.0001
            )
            + pynutil.insert('"')
        )
