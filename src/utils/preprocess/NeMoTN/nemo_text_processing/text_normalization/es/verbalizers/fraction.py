...
class FractionFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        super().__init__(
            name="fraction", kind="verbalize", deterministic=deterministic
        )
        ...
        denominator_singular = pynini.union(
            denominator_add_stem, denominator_ordinal
        )
        ...
        merge @= pynini.cdrewrite(
            delete_space, "", pynini.difference(NEMO_CHAR, "parte"), NEMO_SIGMA
        )

        # The merger can produce duplicate vowels. This is not allowed in orthography
        delete_duplicates = pynini.string_map(
            [("aa", "a"), ("oo", "o")]
        )  # Removes vowels
        delete_duplicates = pynini.cdrewrite(
            delete_duplicates, "", "", NEMO_SIGMA
        )
        ...
        fraction_with_cardinal = strip_cardinal_apocope(
            numerator | numerator_one
        )
        ...
        if not deterministic:
            ...
            vowel_change = pynini.cdrewrite(
                pynini.cross("o", "a"), "", pynini.accep("[EOS]"), NEMO_SIGMA
            )

            denominator_singular_fem = (
                shift_cardinal_gender(denominator_singular)
                @ vowel_change
                @ apply_exceptions
            )
            ...
            merge_stem = pynini.cdrewrite(
                delete_space,
                "",
                pynini.union("avo", "ava", "avos", "avas"),
                NEMO_SIGMA,
            )  # For managing alternative spacing
            merge_stem @= remove_accents @ delete_duplicates

            fraction_with_one_fem = (
                numerator_one_fem + delete_space + insert_space
            )
            ...
            fraction_with_one |= (
                ...
                + delete_space
                + insert_space
                + denominator_singular @ merge_stem
            )
        ...
        graph_masc = (
            optional_sign
            + pynini.closure(integer + delete_space + conjunction, 0, 1)
            + fraction
        )
        ...
        fraction_fem = (
            fraction_with_one | fraction_default | fraction_with_cardinal
        )
        graph_fem = (
            optional_sign
            + pynini.closure(integer_fem + delete_space + conjunction, 0, 1)
            + fraction_fem
        )
