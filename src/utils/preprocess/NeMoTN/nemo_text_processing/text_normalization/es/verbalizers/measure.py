...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        super().__init__(
            name="measure", kind="verbalize", deterministic=deterministic
        )
        ...
        unit_masc = (
            ...
            + (pynini.closure(NEMO_NOT_QUOTE) @ unit_masc)
            + pynutil.delete('"')
        )
        ...
        unit_fem = (
            pynutil.delete('units: "')
            + (pynini.closure(NEMO_NOT_QUOTE) @ unit_fem)
            + pynutil.delete('"')
        )
        ...
        graph_masc |= pynutil.add_weight(
            graph_fraction_masc @ (NEMO_SIGMA + pynini.union("medio", "medios"))
            + NEMO_WHITE_SPACE
            + unit_masc,
            -0.001,
        ...
        graph_fem = (
            (graph_cardinal_fem | graph_decimal_fem)
            + NEMO_WHITE_SPACE
            + unit_fem
        )
        graph_fem |= (
        ...
        graph @= pynini.cdrewrite(
            pynini.cross(ones, "uno"), "", NEMO_WHITE_SPACE + "por", NEMO_SIGMA
        )
