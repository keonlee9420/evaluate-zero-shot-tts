...
class DateFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool):
        ...
        year_with_articles = (
            pynutil.insert('year: "')
            + pynini.closure(articles + NEMO_SPACE, 0, 1)
            + year
            + pynutil.insert('"')
        )
        ...
        for sep in separators:
            year_optional = pynini.closure(
                pynini.cross(sep, NEMO_SPACE) + year_only, 0, 1
            )
            new_graph = (
                day
        ...
        day_optional = pynini.closure(
            pynini.cross(dash, NEMO_SPACE) + day, 0, 1
        )
        graph_ymd = (
            NEMO_DIGIT**4 @ year_only
            + pynini.cross(dash, NEMO_SPACE)
            + month_number
            + day_optional
        )
