...
fraction_symbols = pynini.string_file(
    get_abs_path("data/fractions/fraction_symbols.tsv")
)
...
class FractionFst(GraphFst):
    ...
    def __init__(
        ...
        super().__init__(
            name="fraction", kind="classify", deterministic=deterministic
        )
        ...
        block_three_to_ten = pynutil.delete(
            three_to_ten
        )  # To block cardinal productions
        if (
            not deterministic
        ):  # Multiples of tens are sometimes rendered as ordinals
            three_to_ten |= pynini.string_map(
                [
                    "20",
                    "30",
        ...
        graph_three_to_ten @= pynini.cdrewrite(
        ...
        multiples_of_thousand = (
            ordinal.multiples_of_thousand
        )  # So we can have X milésimos
        ...
        graph_fractions_ordinals = (
            graph_higher_powers_of_ten | graph_three_to_ten
        )
        ...
        fraction = (fraction_symbols @ (numerator + denominator)) | (
        ...
        self.graph = (
            optional_minus_graph + pynini.closure(integer, 0, 1) + fraction
        )
