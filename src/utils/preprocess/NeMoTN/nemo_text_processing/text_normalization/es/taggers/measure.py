...
unit_complex = pynini.string_file(
    get_abs_path("data/measures/measurements_complex.tsv")
)
...
math_symbols = pynini.string_file(
    get_abs_path("data/measures/math_symbols.tsv")
)
...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        super().__init__(
            name="measure", kind="classify", deterministic=deterministic
        )
        ...
        unit_plural = (
            pynutil.insert('units: "') + graph_unit_plural + pynutil.insert('"')
        )
        ...
        unit_complex_plural = unit_complex @ (
            unit_plural_fem | unit_plural_masc
        )
        ...
        complex_unit_singular_graph = (
            pynutil.insert('units: "')
            + (
                (graph_unit_singular + optional_unit_denominator)
                | graph_unit_denominator
                | graph_unit_complex
            )
            + pynutil.insert('"')
        ...
        complex_unit_plural_graph = (
            pynutil.insert('units: "')
            + (
                (graph_unit_plural + optional_unit_denominator)
                | graph_unit_denominator
                | graph_unit_complex_plural
            )
            + pynutil.insert('"')
        )
        ...
        math_numbers = (
            cardinal_graph
            | pynutil.add_weight(cardinal_graph @ pynini.cross("un", "uno"), -1)
            | NEMO_ALPHA
        )
        math = (
            math_numbers
            + pynini.closure(
                delimiter + math_symbols + delimiter + math_numbers, 1
            )
            + delimiter
