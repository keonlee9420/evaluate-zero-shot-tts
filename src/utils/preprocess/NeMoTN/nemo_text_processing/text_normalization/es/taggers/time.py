...
class TimeFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        delete_minute_suffix = (
            delete_space
            + insert_space
            + (pynutil.delete("min") | pynutil.delete("m"))
        )
        delete_second_suffix = (
            delete_space
            + insert_space
            + (pynutil.delete("seg") | pynutil.delete("s"))
        )
        ...
        delete_leading_zero_to_double_digit = (
            pynini.closure(pynutil.delete("0") | (NEMO_DIGIT - "0"), 0, 1)
            + NEMO_DIGIT
        )
        ...
        final_graph_hour_24 = (
            pynutil.insert('hours: "') + graph_hour_24 + pynutil.insert('"')
        )
        final_graph_hour_12 = (
            pynutil.insert('hours: "') + graph_hour_12 + pynutil.insert('"')
        )
        ...
        graph_hm = (
            ...
            + pynini.closure(
                delete_hora_suffix, 0, 1
            )  # 2.30 is valid if unambiguous
