...
def get_quantity(
    ...
    numbers = (
        pynini.cdrewrite(pynutil.delete(cardinal_separator), "", "", NEMO_SIGMA)
        @ numbers
    )
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        if not deterministic:
            ...
            graph = pynini.union(
                ...
                ),  # For cases such as "1,010"
            )

        # Need to strip apocope everywhere BUT end of string
        reverse_apocope = pynini.string_map([("un", "uno"), ("ún", "uno")])
        apply_reverse_apocope = pynini.cdrewrite(
            reverse_apocope, "", NEMO_SPACE, NEMO_SIGMA
        )
        ...
        self.final_graph_wo_negative = (
            final_graph_wo_sign
            | get_quantity(final_graph_wo_sign, cardinal.graph).optimize()
        )
