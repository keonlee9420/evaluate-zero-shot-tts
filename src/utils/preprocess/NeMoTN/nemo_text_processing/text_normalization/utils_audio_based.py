...
def adjust_boundaries(
    ...
    while word_id < len(norm.split()):
        ...
        if (norm_raw[2] == MATCH and norm_pred[2] == NONMATCH) or (
            norm_raw[2] == NONMATCH and norm_pred[2] == MATCH
        ):
            ...
            while word_id < len(norm.split()) and not done:
                norm_raw, norm_pred = (
                    norm_raw_diffs[word_id],
                    norm_pred_diffs[word_id],
                )
    ...
    while idx < len(spans_merged_neighbors):
        ...
        if item[2][0] == item[2][1]:
            # for the last item
            if (
                idx == len(spans_merged_neighbors) - 1
                and len(raw_norm_spans_corrected_for_pred_text) > 0
            ):
                ...
                while idx < len(spans_merged_neighbors) - 1 and not (
                    ...
                    and spans_merged_neighbors[idx][-1] == MATCH
                ):
                    idx += 1
                ...
                extended_spans.append(
                    len(raw_norm_spans_corrected_for_pred_text) - 1
                )
            idx += 1
        else:
    ...
    for idx, item in enumerate(raw_norm_spans_corrected_for_pred_text):
        ...
        if (item[-1] == NONMATCH and cur_semiotic != cur_norm_span) or (
            idx in extended_spans
        ):
    ...
    raw_text_mask_idx = [
        idx for idx, x in enumerate(raw_text_masked_list) if x == SEMIOTIC_TAG
    ]
    ...
    if verbose:
        ...
        for item in spans_merged_neighbors:
            print(
                f"{raw.split()[item[1][0]: item[1][1]]} -- {pred_text.split()[item[2][0]: item[2][1]]}"
            )
