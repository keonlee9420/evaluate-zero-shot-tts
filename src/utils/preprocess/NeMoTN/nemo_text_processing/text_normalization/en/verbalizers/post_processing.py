...
class PostProcessingFst:
    ...
    def get_punct_postprocess_graph(self):
        ...
        no_space_before_punct = [
            m for m in punct_marks_all if m not in allow_space_before_punct
        ]
        ...
        non_punct = pynini.difference(
            NEMO_CHAR, no_space_before_punct
        ).optimize()
        graph = (
            ...
            + pynini.closure(
                no_space_before_punct
                | pynutil.add_weight(
                    delete_space + no_space_before_punct, MIN_NEG_WEIGHT
                )
            )
            + pynini.closure(non_punct)
        )
        ...
        no_space_after_punct = pynini.cdrewrite(
            delete_space, no_space_after_punct, NEMO_SIGMA, NEMO_SIGMA
        ).optimize()
        ...
        not_alpha = pynini.difference(
            NEMO_CHAR, NEMO_ALPHA
        ).optimize() | pynutil.add_weight(NEMO_SPACE, MIN_NEG_WEIGHT)
        ...
        quotes_graph = NEMO_SIGMA + pynini.closure(
            NEMO_SIGMA + quotes_graph + NEMO_SIGMA
        )
        ...
        graph = pynini.compose(
            graph, remove_space_around_single_quote
        ).optimize()
        return graph
