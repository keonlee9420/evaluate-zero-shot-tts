...
from nemo_text_processing.text_normalization.en.taggers.date import (
...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True, lm: bool = False):
        ...
        graph = pynini.Far(
            get_abs_path("data/number/cardinal_number_name.far")
        ).get_fst()
        graph_au = pynini.Far(
            get_abs_path("data/number/cardinal_number_name_au.far")
        ).get_fst()
        ...
        if not deterministic:
            ...
            self.single_digits_graph = (
                ...
                + pynini.closure(insert_space + single_digits_graph_zero)
            )
        ...
        if deterministic:
            long_numbers = pynini.compose(
                NEMO_DIGIT ** (5, ...), self.single_digits_graph
            ).optimize()
            self.long_numbers = plurals._priority_union(
                long_numbers, self.graph_with_and, NEMO_SIGMA
            ).optimize()
            ...
            final_graph |= pynini.compose(
                final_graph, one_to_a_replacement_graph.optimize() + NEMO_SIGMA
            ).optimize()
            # remove commas for 4 digits numbers
            four_digit_comma_graph = (
                (NEMO_DIGIT - "0") + pynutil.delete(",") + NEMO_DIGIT**3
            )
            final_graph |= pynini.compose(
    ...
    def add_optional_and(self, graph):
        ...
        if not self.lm:
            ...
            no_thousand_million = pynini.difference(
                not_quote,
                not_quote + pynini.union("thousand", "million") + not_quote,
            ).optimize()
            integer = (
                not_quote
                + pynutil.add_weight(
            ...
            no_hundred = pynini.difference(
                NEMO_SIGMA, not_quote + pynini.accep("hundred") + not_quote
            ).optimize()
            integer |= (
                not_quote
                + pynutil.add_weight(
            ...
            optional_hundred = pynini.compose(
                (NEMO_DIGIT - "0") ** 3, graph
            ).optimize()
            optional_hundred = pynini.compose(
                optional_hundred,
                NEMO_SIGMA + pynini.cross(" hundred", "") + NEMO_SIGMA,
            )
