...
class OrdinalFst(GraphFst):
    ...
    def __init__(
        self, number_names: dict, alternative_formats: dict, deterministic=False
    ):
        super().__init__(
        ...
        endings = pynini.string_file(
            get_abs_path("data/numbers/ordinal_endings.tsv")
        )
        not_dash = pynini.closure(pynini.difference(NEMO_SIGMA, "-"))
        del_ending = pynini.cdrewrite(
            pynini.cross("-" + not_dash, ""), "", "[EOS]", NEMO_SIGMA
        )
        ordinal_numbers_marked = (
            (
                (separators @ ordinal).optimize() + pynini.accep("-") + not_dash
            ).optimize()
        ...
        self.ordinal_numbers_with_leading_zeros = (
            leading_zeros + ordinal_numbers
        ).optimize()

        final_graph = (ordinal_numbers | ordinal_numbers_marked).optimize()
        final_graph = (
            pynutil.insert('integer: "') + final_graph + pynutil.insert('"')
        )
