...
class CardinalFst(GraphFst):
    ...
    def __init__(
        ...
        optional_quantity = pynini.string_file(
        ...
        final_graph |= pynini.compose(
            pynini.compose(
                NEMO_DIGIT ** (1, ...) + pynini.cross("-х", ""), final_graph
            ),
    ...
    def get_serial_graph(self):
        ...
        delimiter = (
            insert_space | pynini.cross("-", " ") | pynini.cross("/", " ")
        )
        ...
        serial_graph = (
            letter_num | num_letter | num_delimiter_num
        ) + next_alpha_or_num
        ...
        serial_graph = pynini.compose(
            at_least_one_alpha_num, serial_graph.optimize()
        ).optimize()
