...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        delete_space = pynini.closure(
            pynutil.add_weight(
                pynutil.delete(
                    pynini.union(NEMO_SPACE, NEMO_NON_BREAKING_SPACE)
                ),
                -1,
            ),
            0,
        ...
        slash_unit = (
            (pynini.cross("/", "в") | pynini.cross("/", "за"))
            + space_for_units
            + graph_unit
        )
        ...
        default_units = (
            ...
            + (graph_unit | unit_slash_unit)
            + pynutil.insert('"')
        )
        slash_units = (
            pynutil.insert('units: "') + slash_unit + pynutil.insert('"')
        )
        ...
        self.tagger_graph_default = (
            subgraph_decimal | cardinal_space
        ).optimize()
