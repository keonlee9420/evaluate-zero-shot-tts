...
class DateFst(GraphFst):
    ...
    def __init__(self, number_names: dict, deterministic: bool):
        ...
        day = pynini.compose(
            ...
            pynini.difference(
                NEMO_SIGMA,
                NEMO_SIGMA + pynini.union("ой", "ая", "ых", "ые", "ыми"),
            ),
        )
        ...
        year = pynini.compose(
            ((NEMO_DIGIT**4) | (NEMO_DIGIT**2)), numbers
        ).optimize()
        year |= zero_digit

        # reduce year options
        year_wrong_endings = ["ую", "ая"]
        year_wrong_beginning = [
            "две тысяча",
            "два тысяч",
            "два тысячи",
            "две тысяч ",
        ...
        year = pynini.compose(
            year,
            pynini.difference(
                NEMO_SIGMA, NEMO_SIGMA + pynini.union("ой", "ого")
            ),
        ) | pynutil.add_weight(
            pynini.compose(year, NEMO_SIGMA + pynini.union("ой", "ого")), -0.1
        )

        year_restrict1 = pynini.difference(
            NEMO_SIGMA, pynini.union(*year_wrong_beginning) + NEMO_SIGMA
        )
        year_restrict2 = pynini.difference(
            NEMO_SIGMA, NEMO_SIGMA + pynini.union(*year_wrong_endings)
        )
        year = pynini.compose(
            pynini.compose(year, year_restrict1), year_restrict2
        )
        ...
        year_word = (
            pynutil.add_weight(insert_space, -0.1)
            | pynutil.add_weight(pynini.accep(" "), 0.1)
        ) + year_word

        year_optional = (
            pynutil.insert('year: "')
            + year
            + pynini.closure(year_word, 0, 1)
            + pynutil.insert('"')
        )
        year_optional = pynini.closure(
            delete_sep + year_optional, 0, 1
        ).optimize()
        year_only = (
            pynutil.insert('year: "') + year + year_word + pynutil.insert('"')
        )
        ...
        self.final_graph = pynini.compose(
            tagger_graph, verbalizer_graph
        ).optimize()
        self.fst = (
            pynutil.insert('day: "') + self.final_graph + pynutil.insert('"')
        )
