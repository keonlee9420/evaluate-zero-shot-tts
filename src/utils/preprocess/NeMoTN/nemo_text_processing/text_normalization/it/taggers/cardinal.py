...
tens_one = pynini.invert(
    pynini.string_file(get_abs_path("data/numbers/tens_one.tsv"))
)
...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        graph_million = pynutil.add_weight(
            pynini.cross("000001", "un milione"), -0.001
        )
        graph_million |= (
            graph_thousands_component_at_least_one_none_zero_digit_no_one
            + pynutil.insert(" milioni")
        )
        graph_million |= pynutil.delete("000000")
        graph_million += insert_space

        graph_billion = pynutil.add_weight(
            pynini.cross("000001", "un miliardo"), -0.001
        )
        graph_billion |= (
            graph_thousands_component_at_least_one_none_zero_digit_no_one
            + pynutil.insert(" miliardi")
        )
        graph_billion |= pynutil.delete("000000")
        graph_billion += insert_space

        graph_trillion = pynutil.add_weight(
            pynini.cross("000001", "un trilione"), -0.001
        )
        graph_trillion |= (
            graph_thousands_component_at_least_one_none_zero_digit_no_one
            + pynutil.insert(" trilioni")
        )
        ...
        graph = (
            ...
            + graph_million
            + (
                graph_thousands_component_at_least_one_none_zero_digit
