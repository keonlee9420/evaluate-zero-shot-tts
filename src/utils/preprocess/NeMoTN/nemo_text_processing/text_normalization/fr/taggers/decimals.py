...
def get_quantity(
    ...
    numbers = (
        pynini.cdrewrite(pynutil.delete(delete_separator), "", "", NEMO_SIGMA)
        @ numbers
    )
...
class DecimalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        super().__init__(
            name="decimal", kind="classify", deterministic=deterministic
        )
        ...
        if not deterministic:
            graph = pynini.union(
                graph_digit, cardinal.all_hundreds, cardinal.one_to_all_tens
            )
            ...
            graph = pynini.union(
                pynutil.add_weight(
                    graph_digit + pynini.closure(insert_space + zero), -0.00001
                ),
                pynutil.add_weight(
                ...
                pynutil.add_weight(
                    cardinal.all_hundreds + pynini.closure(insert_space + zero),
                    0.00001,
                ),
                pynutil.add_weight(
                    cardinal.all_double_digits
                    + pynini.closure(
                        insert_space + cardinal.all_double_digits, 1
                    )
                    + pynini.closure(insert_space + zero, 0, 1)
                    + (
                        pynini.closure(insert_space + graph_digit, 0, 1)
                        | pynini.closure(insert_space + zero, 0)
                    ),
                    -0.00002,
        ...
        graph_fractional = (
            pynutil.insert('fractional_part: "') + graph + pynutil.insert('"')
        )

        integers = cardinal.all_nums_no_tokens
        graph_integer = (
            pynutil.insert('integer_part: "') + integers + pynutil.insert('"')
        )
        final_graph_wo_sign = (
            graph_integer + delete_separator + insert_space + graph_fractional
        )

        final_graph_wo_negative = (
            final_graph_wo_sign
            | get_quantity(final_graph_wo_sign, integers).optimize()
        )
