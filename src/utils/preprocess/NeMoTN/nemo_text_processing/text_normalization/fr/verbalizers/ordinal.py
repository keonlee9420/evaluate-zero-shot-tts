...
class OrdinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True, strip_dashes: bool = False):
        super().__init__(
            name="ordinal", kind="verbalize", deterministic=deterministic
        )

        ones = pynini.cross("un", "prem")

        irregular_numbers = pynini.string_file(
            get_abs_path("data/ordinals/irregular_numbers.tsv")
        )
        irregular_numbers = (
            pynini.closure(pynini.closure(NEMO_NOT_QUOTE, 1) + NEMO_SPACE)
            + irregular_numbers
        )
        ...
        if strip_dashes:
            ...
            numbers = pynini.union(
                numbers, pynutil.add_weight(numbers @ remove_dashes, -0.0001)
            )

        regular_ordinals = pynini.union(
            numbers, pynutil.add_weight(ones, -0.0001)
        )
        ...
        final_graph = pynini.union(
            regular_ordinals_graph, irregular_numbers_graph
        )
