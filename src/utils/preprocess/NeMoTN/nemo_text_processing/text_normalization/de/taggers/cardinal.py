...
import pynini
from nemo_text_processing.text_normalization.de.utils import (
    get_abs_path,
    load_labels,
)
...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = False):
        ...
        def hundred_non_zero():
            return (
                graph_digit_no_one + insert_space | pynini.cross("1", "ein ")
            ) + pynutil.insert("hundert") + (
                pynini.closure(
                    insert_space + pynutil.insert(AND, weight=0.0001), 0, 1
                )
                + insert_space
                + tens_no_zero()
                | pynutil.delete("00")
        ...
        optional_plural_quantity_en = pynini.closure(
            pynutil.insert("en", weight=-0.0001), 0, 1
        )
        optional_plural_quantity_n = pynini.closure(
            pynutil.insert("n", weight=-0.0001), 0, 1
        )
