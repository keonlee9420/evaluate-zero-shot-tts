...
unit_singular = pynini.string_file(
    get_abs_path("data/measure/measurements.tsv")
)
...
def singular_to_plural():
    ...
    _nen = (
        NEMO_SIGMA
        + pynini.union("in")
        + (pynutil.insert("e") | pynutil.insert("nen"))
    )
    ...
    _s = (
        NEMO_SIGMA + pynini.union("a", "i", "o", "u", "y") + pynutil.insert("s")
    )
...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        graph_unit_plural = graph_unit_singular @ pynini.cdrewrite(
            convert_space(suppletive), "", "[EOS]", NEMO_SIGMA
        )
