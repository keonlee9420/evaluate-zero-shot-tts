...
class DateFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True, lm: bool = False):
        ...
        delete_day = (
            pynutil.delete("号") | pynutil.delete("號") | pynutil.delete("日")
        )
        ...
        only_day = (
            pynutil.insert('day: "') + day + delete_day + pynutil.insert('"')
        )
        ...
        prefix_component = (
            pynutil.insert('era: "') + prefix + pynutil.insert('"')
        )
        graph_prefix = (
            prefix_component
            + pynutil.insert(" ")
            + (pynutil.add_weight(graph_all, -2.0))
        )

        suffix_component = (
            pynutil.insert('era: "') + suffix + pynutil.insert('"')
        )
        graph_suffix = (
            (pynutil.add_weight(graph_all, -2.0))
            + pynutil.insert(" ")
        ...
        graph_suffix_year = (
            graph_suffix_year + pynutil.insert(" ") + suffix_component
        )

        graph_with_era = graph_suffix_year | graph_affix

        graph = (
            graph_only_date | graph_combination | graph_sign | graph_with_era
        )
