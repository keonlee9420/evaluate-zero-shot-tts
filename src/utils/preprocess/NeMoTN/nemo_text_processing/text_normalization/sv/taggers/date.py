...
class DateFst(GraphFst):
    ...
    def __init__(
        self, cardinal: GraphFst, ordinal: GraphFst, deterministic: bool = True
    ):
        super().__init__(
        ...
        month_numbers = pynini.string_file(
            get_abs_path("data/dates/months.tsv")
        )
        month_abbr = pynini.string_file(
            get_abs_path("data/dates/month_abbr.tsv")
        )
        era_suffix = pynini.string_file(
            get_abs_path("data/dates/era_suffix.tsv")
        )
        ...
        # 01, 31, 1
        self.digit_day = (
            pynini.union(*[str(x) for x in range(1, 32)])
            @ ordinal.bare_ordinals
        )
        ...
        self.digit_day_zero = (
            pynini.project(digit_day, "input")
            - pynini.project((NEMO_DIGIT @ self.digit_day), "input")
        ) @ digit_day
        digit_day |= NEMO_DIGIT @ self.digit_day
        digit_words = pynini.project(digit_day, "output")
        day_only = (
            pynutil.insert('day: "') + digit_day + pynutil.insert('"')
        ).optimize()
        day = (
            pynutil.insert('day: "')
            + digit_day
        ...
        day_sfx = (
            pynutil.insert('day: "')
            + ordinal.suffixed_to_words
            + pynutil.insert('"')
        ).optimize()
        day_words = (
            pynutil.insert('day: "') + digit_words + pynutil.insert('"')
        ).optimize()
        ...
        year_first = (
            (NEMO_DIGIT - "0") + pynini.closure(NEMO_DIGIT, 0, 1)
        ) @ numbers
        ...
        year_cardinal = (
            (NEMO_DIGIT - "0") + pynini.closure(NEMO_DIGIT, 1, 3)
        ) @ numbers
        year = pynini.union(
            year_first + year_second, year_first
        )  # 90, 990, 1990
        if not deterministic:
            year |= year_cardinal
            year |= year_hundra
        ...
        decade_num = pynini.union(
            year_first + year_second_decades, year_second_decades
        )
        ...
        if not deterministic:
            ...
            decade_num |= (
                (NEMO_DIGIT - "0") + pynini.closure(NEMO_DIGIT, 1, 2) + "0"
            ) @ numbers
        decade = (
            decade_num + tal_hyphen + (decade_word | tals_word)
        ).optimize()
        ...
        year_opt_era = (
            year_only + optional_era + pynutil.insert(" preserve_order: true")
        )
        ...
        for sep in separators:
            day_optional = pynini.closure(
                pynini.cross(sep, NEMO_SPACE) + day_only, 0, 1
            )
            year_optional = pynini.closure(
                pynini.cross(sep, NEMO_SPACE) + year_only + optional_era
            )
            new_graph = (
                day_only
                + pynini.cross(sep, NEMO_SPACE)
                + month_number
                + year_optional
            )
            graph_dmy |= new_graph
