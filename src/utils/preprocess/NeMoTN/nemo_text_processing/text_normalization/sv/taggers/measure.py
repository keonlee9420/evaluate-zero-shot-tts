...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        graph_unit_ett = pynini.string_file(
            get_abs_path("data/measure/unit_neuter.tsv")
        )
        graph_plurals = pynini.string_file(
            get_abs_path("data/measure/unit_plural.tsv")
        )
        greek_lower = pynini.string_file(
            get_abs_path("data/measure/greek_lower.tsv")
        )
        greek_upper = pynutil.insert("stort ") + pynini.string_file(
            get_abs_path("data/measure/greek_upper.tsv")
        )
        ...
        graph_unit |= pynini.compose(
            ...
            + (SV_ALPHA | TO_LOWER)
            + pynini.closure(SV_ALPHA | TO_LOWER),
            graph_unit,
        ).optimize()
        ...
        unit_graph = (
            ...
            + (
                (pynini.cross("/", "per") + delete_zero_or_one_space)
    ...
    def get_range(self, cardinal: GraphFst):
        ...
        range_graph = (
            cardinal
            + pynini.cross(pynini.union("-", " - "), " till ")
            + cardinal
        )
