...
def make_million(
    number: str, non_zero_no_one: "pynini.FstLike", deterministic: bool = True
) -> "pynini.FstLike":
    """
    ...
    if not deterministic:
        ...
        for one in ["en", "ett"]:
            graph |= pynutil.add_weight(
                pynini.cross("001", f"{one} {number}"), -0.001
            )
            graph |= pynutil.add_weight(
                pynini.cross("001", f"{one} {old_orth}"), -0.001
            )
            graph |= pynutil.add_weight(
                pynini.cross("001", f"{one}{number}"), -0.001
            )
    ...
    if not deterministic:
        graph |= pynutil.add_weight(
            non_zero_no_one + pynutil.insert(f" {old_orth}er"), -0.001
        )
        graph |= pynutil.add_weight(
            non_zero_no_one + pynutil.insert(f"{old_orth}er"), -0.001
        )
...
def filter_punctuation(fst: "pynini.FstLike") -> "pynini.FstLike":
    ...
    cardinal_string |= (
        ...
        + pynini.closure(
            exactly_three_digits + pynutil.delete(cardinal_separator)
        )
...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        super().__init__(
            name="cardinal", kind="classify", deterministic=deterministic
        )
        zero = pynini.invert(
        ...
        alt_ties = ties @ pynini.cdrewrite(
            ties_alt_endings, "", "[EOS]", NEMO_SIGMA
        )
        if not deterministic:
        ...
        if deterministic:
            ...
            graph_tens |= pynutil.add_weight(
                pynini.cross("18", "aderton"), -0.001
            )
            graph_tens |= pynutil.add_weight(
                graph_ties
                + (
                    pynutil.delete("0")
                    | (graph_digit | insert_space + graph_digit)
                ),
                -0.001,
            )
        ...
        if not deterministic:
            hundreds |= pynutil.add_weight(
                pynini.cross("1", "etthundra"), -0.001
            )
            hundreds |= pynutil.add_weight(
                digit + pynutil.insert(NEMO_SPACE) + pynutil.insert("hundra"),
                -0.001,
            )
        ...
        self.final_two_digit_non_zero = (
            graph_final_two_digit_non_zero.optimize()
        )
        ...
        graph_hundreds_component_at_least_one_non_zero_digit = (
            graph_hundreds_component | (pynutil.delete("00") + graph_digit)
        )

        graph_hundreds_component_at_least_one_non_zero_digit_no_one = (
            graph_hundreds_component | (pynutil.delete("00") + digits_no_one)
        )
        ...
        if not deterministic:
            ...
            etttusen |= pynutil.add_weight(
                pynini.cross("001", " tusen"), -0.001
            )
            etttusen |= pynutil.add_weight(
                pynini.cross("001", "etttusen"), -0.001
            )
            etttusen |= pynutil.add_weight(
                pynini.cross("001", " etttusen"), -0.001
            )
            etttusen |= pynutil.add_weight(
                pynini.cross("001", "ett tusen"), -0.001
            )
            etttusen |= pynutil.add_weight(
                pynini.cross("001", " ett tusen"), -0.001
            )
        ...
        following_hundred = (
            insert_space + graph_hundreds_component_at_least_one_non_zero_digit
        )
        if not deterministic:
            following_hundred |= (
                graph_hundreds_component_at_least_one_non_zero_digit
        ...
        graph_trilliard = make_million(
            "triljard", non_zero_no_one, deterministic
        )
        ...
        self.graph = (
            ...
            @ pynini.cdrewrite(
                pynini.cross(pynini.closure(NEMO_WHITE_SPACE, 2), NEMO_SPACE),
                SV_ALPHA,
                SV_ALPHA,
                NEMO_SIGMA,
            )
        ...
        self.graph_hundreds_component_at_least_one_non_zero_digit = (
            ...
            | pynini.difference(NEMO_DIGIT, pynini.accep("0"))
        ) @ self.graph
        ...
        self.two_or_three_digits_read_frac = pynini.union(
            ...
            zero_space
            + single_digits_graph
            + pynini.closure(insert_space + digit, 0, 1),
            single_digits_graph
            + pynini.closure(insert_space + single_digits_graph, 3),
        ...
        self.two_or_three_digits_read_frac_en = pynini.union(
            ...
            zero_space
            + single_digits_graph
            + pynini.closure(insert_space + single_digits_graph, 0, 1),
            single_digits_graph
            + pynini.closure(insert_space + single_digits_graph, 3),
            zero_space + zero_space + zero,
            single_digits_graph,
        ...
        self.two_digits_read = pynini.union(
            ((NEMO_DIGIT - "0") + NEMO_DIGIT) @ graph_tens, zero_space + digit
        )
        ...
        self.any_read_digit = ((NEMO_DIGIT - "0") @ digit) + pynini.closure(
            insert_space + digit
        )
        if not deterministic:
            self.three_digits_read |= pynutil.add_weight(
                digit + insert_space + digit + insert_space + digit, -0.001
            )
            self.three_digits_read |= pynutil.add_weight(
                ((NEMO_DIGIT - "0") + NEMO_DIGIT) @ graph_tens
                + insert_space
            ...
            self.two_digits_read |= pynutil.add_weight(
                digit + insert_space + digit, -0.001
            )
        ...
        self.graph_en = self.graph @ pynini.cdrewrite(
            ett_to_en, "", "[EOS]", NEMO_SIGMA
        )
        self.graph_no_one = (
