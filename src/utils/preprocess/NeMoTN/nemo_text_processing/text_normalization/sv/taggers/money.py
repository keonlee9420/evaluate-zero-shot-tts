...
from nemo_text_processing.text_normalization.sv.graph_utils import (
    SV_ALPHA,
    ensure_space,
)
...
class MoneyFst(GraphFst):
    ...
    def __init__(
        ...
        super().__init__(
            name="money", kind="classify", deterministic=deterministic
        )
        ...
        maj_singular_nt = pynini.string_file(
            get_abs_path("data/money/currency_major_nt.tsv")
        )
        maj_plural = pynini.string_file(
            get_abs_path("data/money/currency_plurals.tsv")
        )

        maj_singular_labels = load_labels(
            get_abs_path("data/money/currency_major.tsv")
        )
        maj_singular_labels_nt = load_labels(
            get_abs_path("data/money/currency_major_nt.tsv")
        )
        ...
        graph_maj_singular_nt = (
            pynutil.insert('currency_maj: "')
            + maj_unit_singular_nt
            + pynutil.insert('"')
        )
        graph_maj_plural_nt = (
            pynutil.insert('currency_maj: "')
        ...
        graph_integer_only = (
            graph_maj_singular + ensure_space + graph_integer_sg_en
        )
        graph_integer_only |= (
            graph_maj_singular_nt + ensure_space + graph_integer_sg_ett
        )
        graph_integer_only |= graph_maj_plural + ensure_space + graph_integer
        graph_integer_only |= (
            graph_maj_plural_nt + ensure_space + graph_integer_ett
        )
        graph_integer_only |= (
            graph_integer_sg_en + ensure_space + graph_maj_singular
        )
        graph_integer_only |= (
            graph_integer_sg_ett + ensure_space + graph_maj_singular_nt
        )
        graph_integer_only |= graph_integer + ensure_space + graph_maj_plural
        graph_integer_only |= (
            graph_integer_ett + ensure_space + graph_maj_plural_nt
        )
        ...
        maj_singular_labels_all += [
            (x[0], "ett") for x in maj_singular_labels_nt
        ]
        # format ** dollars ** cent
        ...
        for curr_symbol, one_form in maj_singular_labels_all:
            ...
            if one_form == "en":
                ...
                integer_plus_maj = (
                    graph_integer_ett
                    + insert_space
                    + (pynutil.insert(curr_symbol) @ graph_maj_plural_nt)
                )
                integer_plus_maj |= (
                    graph_integer_sg_ett
                    + insert_space
                    + (pynutil.insert(curr_symbol) @ graph_maj_singular_nt)
                )

            integer_plus_maj_with_comma = pynini.compose(
                NEMO_DIGIT - "0" + pynini.closure(NEMO_DIGIT | delete_space),
                integer_plus_maj,
            )
