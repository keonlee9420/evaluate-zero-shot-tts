...
class TelephoneFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        super().__init__(
            name="telephone", kind="classify", deterministic=deterministic
        )
        ...
        zero_after_country_code = pynini.union(
            pynini.cross("(0)", "noll "), zero_space
        )
        bracketed = (
            pynutil.delete("(") + one_two_or_three_digits + pynutil.delete(")")
        )

        zero = pynini.cross("0", "noll")
        digit |= zero

        special_numbers = pynini.string_file(
            get_abs_path("data/telephone/special_numbers.tsv")
        )

        passable = pynini.union(":", ": ", " ")
        prompt_pass = pynini.closure(
            pynutil.delete(passable) + insert_space, 0, 1
        )
        telephone_abbr = pynini.string_file(
            get_abs_path("data/telephone/telephone_abbr.tsv")
        )
        telephone_abbr = telephone_abbr + prompt_pass
        telephone_prompt = pynini.string_file(
            get_abs_path("data/telephone/telephone_prompt.tsv")
        )
        prompt_as_code = (
        ...
        country_code = (
            pynutil.insert('country_code: "') + country + pynutil.insert('"')
        )
        ...
        ext_prompt = (
            NEMO_SPACE
            + pynutil.delete(pynini.union("ankn", "ankn.", "anknytning"))
            + ensure_space
        )
        passable = pynini.union(":", ": ", " ")
        ...
        three_digit_area_code_no_zero = (
            three_digit_area_code_digit_two + NEMO_DIGIT
        ) @ cardinal.two_digits_read
        three_digit_area_code = zero_space + three_digit_area_code_no_zero
        four_digit_area_code_digit_two = pynini.union("5", "6", "9")
        four_digit_area_code_no_zero = (
            four_digit_area_code_digit_two + NEMO_DIGIT
        ) @ cardinal.three_digits_read
        four_digit_area_code = zero_space + four_digit_area_code_no_zero
        two_digit_area_code = "08" @ cardinal.two_digits_read
        self.area_codes = (
            two_digit_area_code | three_digit_area_code | four_digit_area_code
        )
        ...
        country_code_lead = pynini.cross("+", "plus") | pynini.cross(
            "00", "noll noll"
        )
        raw_country_codes = pynini.string_file(
            get_abs_path("data/telephone/country_codes.tsv")
        )
        self.country_code = (
            country_code_lead
            + insert_space
            + (raw_country_codes @ cardinal.any_read_digit)
        )
        self.country_plus_area_code = (
            self.country_code + NEMO_SPACE + self.area_codes_no_zero
        )

        # ip
        ip_prompts = pynini.string_file(
            get_abs_path("data/telephone/ip_prompt.tsv")
        )
        ip_graph = (
            one_two_or_three_digits
            + (pynini.cross(".", " punkt ") + one_two_or_three_digits) ** 3
        )
        graph |= (
            pynini.closure(
                pynutil.insert('country_code: "')
        ...
        ssn_prompts = pynini.string_file(
            get_abs_path("data/telephone/ssn_prompt.tsv")
        )
