...
class OrdinalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        zero = pynini.invert(
            pynini.string_file(get_abs_path("data/ordinals/zero.tsv"))
        )
        card_ties = pynini.invert(
            pynini.string_file(get_abs_path("data/numbers/ties.tsv"))
        )
        card_digit = pynini.invert(
        ...
        cardinal_format = (NEMO_DIGIT - "0") + pynini.closure(
            digit_or_space + NEMO_DIGIT, 0, 1
        )
        a_format = (
            (
        ...
        if not deterministic:
            graph_hundreds |= hundreds + pynini.union(
                (graph_teens | pynutil.insert(NEMO_SPACE) + graph_teens),
        ...
        kapitlet_word = pynini.union(
            "kapitlet", pynini.cross("kap", "kapitlet")
        )
