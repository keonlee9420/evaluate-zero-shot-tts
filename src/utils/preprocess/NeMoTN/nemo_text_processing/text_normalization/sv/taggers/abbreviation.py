...
class AbbreviationFst(GraphFst):
    ...
    def __init__(self, whitelist: "pynini.FstLike", deterministic: bool = True):
        super().__init__(
            name="abbreviation", kind="classify", deterministic=deterministic
        )

        dot = pynini.accep(".")
        # A.B.C. -> A. B. C.
        graph = (
            SV_UPPER + dot + pynini.closure(insert_space + SV_UPPER + dot, 1)
        )
        # A.B.C. -> A.B.C.
        ...
        if whitelist is not None:
            graph = pynini.compose(
                pynini.difference(
                    pynini.project(graph, "input"),
                    pynini.project(whitelist.graph, "input"),
                ),
                graph,
            )

        graph = (
            pynutil.insert('value: "') + graph.optimize() + pynutil.insert('"')
        )
