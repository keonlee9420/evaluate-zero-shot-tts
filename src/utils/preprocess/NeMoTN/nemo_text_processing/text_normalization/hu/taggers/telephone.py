...
class TelephoneFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        area_codes = (
            pynini.string_file(get_abs_path("data/telephone/area_codes.tsv"))
            @ cardinal.graph
        )
        self.area_codes = area_codes
        country_codes = (
            pynini.string_file(get_abs_path("data/telephone/country_codes.tsv"))
            @ cardinal.graph
        )
        ...
        ip_graph = (
            up_to_three_digits
            + (pynini.cross(".", " pont ") + up_to_three_digits) ** 3
        )
        graph |= (
            pynini.closure(
                pynutil.insert('country_code: "')
