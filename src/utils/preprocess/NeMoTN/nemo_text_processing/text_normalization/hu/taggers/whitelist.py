...
class WhiteListFst(GraphFst):
    ...
    def __init__(
        ...
        super().__init__(
            name="whitelist", kind="classify", deterministic=deterministic
        )
        ...
        graph_inflected = load_inflected(
            get_abs_path("data/whitelist_inflect.tsv"), input_case, False
        )
        graph_inflected_sg = load_inflected(
            get_abs_path("data/whitelist_inflect_sg.tsv"), input_case, True
        )
        units_graph = load_inflected(
            get_abs_path("data/measures/measurements.tsv"), input_case, False
        )
        ...
        self.fst = (
            pynutil.insert('name: "') + self.final_graph + pynutil.insert('"')
        ).optimize()
