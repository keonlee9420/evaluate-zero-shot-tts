...
from nemo_text_processing.text_normalization.hu.graph_utils import (
    TO_LOWER,
    TO_UPPER,
)
...
class DateFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool):
        ...
        month_romans = pynini.string_file(
            get_abs_path("data/dates/months_roman.tsv")
        ).optimize()
        ...
        day_pieces = (
            (digit_day + optional_dot) | day_words | graph_days_suffixed
        )
        day_part = (
            pynutil.insert('day: "') + day_pieces + pynutil.insert('"')
        ).optimize()
        ...
        month_component = (
            pynutil.insert('month: "') + month_part + pynutil.insert('"')
        ).optimize()
        month_number_only = (
        ...
        year = (NEMO_DIGIT - "0") + pynini.closure(
            NEMO_DIGIT, 1, 3
        )  # 90, 990, 1990
        year @= numbers
        self.year = year
        ...
        graph_ymd = (
            optional_year_dot_space + month_component + NEMO_SPACE + day_part
        )
        graph_ymd |= (
        ...
        for sep in separators:
            ...
            if not deterministic:
                ...
                new_graph = (
                    day + pynini.cross(sep, " ") + month_number_only + year_only
                )
            graph_dmy |= new_graph
