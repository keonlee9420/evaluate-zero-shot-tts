...
class TimeFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        super().__init__(
            name="time", kind="classify", deterministic=deterministic
        )

        ora_word = pynini.cross("ó", "óra") | pynini.accep("óra")
        ora_forms = pynini.string_map(
            naive_inflector("ó", "óra", True) + [("ó", "óra")]
        )
        ora_forms_both = ora_forms | pynini.project(ora_forms, "output")
        perc_word = pynini.cross("p", "perc") | pynini.accep("perc")
        perc_forms = pynini.string_map(
            naive_inflector("p", "perc", True) + [("p", "perc")]
        )
        ...
        masodperc_forms = pynini.string_map(
            naive_inflector("mp", "másodperc", True) + [("mp", "másodperc")]
        )
        masodperc_forms_both = masodperc_forms | pynini.project(
            masodperc_forms, "output"
        )
        final_forms = ora_forms_both | perc_forms_both | masodperc_forms_both
        final_suffix = (
            pynutil.insert('suffix: "') + final_forms + pynutil.insert('"')
        )
        ...
        ora_suffix = (
            pynutil.insert('suffix: "') + ora_forms_both + pynutil.insert('"')
        )
        perc_suffix = (
            pynutil.insert('suffix: "')
            + (ora_forms_both | perc_forms_both)
            + pynutil.insert('"')
        )
        ...
        minutes_inverse = pynini.invert(
            pynini.project(minutes_to, "input") @ cardinal.graph
        )
        self.minute_words_to_words = (
            minutes_inverse @ minutes_to @ cardinal.graph
        )
        ...
        hours_next = pynini.string_map(
            [(str(x[0]), str(x[1])) for x in hours_to_pairs()]
        )
        hours_next_inverse = pynini.invert(
            pynini.project(hours_next, "input") @ cardinal.graph
        )
        self.hour_numbers_to_words = hours_next @ cardinal.graph
        self.hour_words_to_words = (
            hours_next_inverse @ self.hour_numbers_to_words
        )
        hour_numbers_to_words = (
            pynutil.insert('hours: "')
            + self.hour_numbers_to_words
            + pynutil.insert('"')
        )
        hour_words_to_words = (
            pynutil.insert('hours: "')
        ...
        quarter_map = pynini.string_map(
            [(p[1], str(p[0])) for p in QUARTERS.items()]
        )
        quarter_map_graph = (
            pynutil.insert('minutes: "')
            + (quarter_map @ cardinal.graph)
            + pynutil.insert('"')
        )
        ...
        self.quarter_prefixed_next_to_current = (
            quarter_map_graph + NEMO_SPACE + hour_numbers_to_words
        )
        # For ITN
        self.quarter_prefixed_next_to_current_words = (
            quarter_map_graph + NEMO_SPACE + hour_words_to_words
        )
        ...
        final_graph_minute_maybe_zero = (
            pynutil.insert('minutes: "')
            + (
                pynini.closure(pynutil.delete("0"), 0, 1) + graph_minute_single
                | graph_minute_double
            )
            @ cardinal.graph
            + pynutil.insert('"')
