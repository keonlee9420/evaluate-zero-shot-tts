...
class MoneyFst(GraphFst):
    ...
    def __init__(
        ...
        maj_singular_labels = load_labels(
            get_abs_path("data/money/currency.tsv")
        )
        ...
        for curr_symbol, cur_word in maj_singular_labels:
            ...
            abbr_expansion = pynini.string_map(
                naive_inflector(curr_symbol, cur_word)
            )
            ...
            if re.match("^[A-Z]{3}$", curr_symbol):
                letter_expansion = pynini.string_map(
                    inflect_abbreviation(curr_symbol, cur_word)
                )
                ...
                if not deterministic:
                    ...
                    letter_endings = get_endings @ (
                        pynini.cdrewrite(
                            pynini.cross(f"{curr_symbol}-", expanded),
                            "[BOS]",
                            "",
                            NEMO_SIGMA,
                        )
            ...
            graph |= (
                graph_integer + ensure_space + graph_maj_final + preserve_order
            )
