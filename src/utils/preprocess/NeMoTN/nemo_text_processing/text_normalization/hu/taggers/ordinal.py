...
class OrdinalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic=False):
        super().__init__(
            name="ordinal", kind="classify", deterministic=deterministic
        )

        endings = pynini.string_file(get_abs_path("data/ordinals/endings.tsv"))
        exceptions = pynini.string_file(
            get_abs_path("data/ordinals/exceptional.tsv")
        )
        superessive_endings = pynini.string_file(
            get_abs_path("data/ordinals/superessive_endings.tsv")
        )
        superscript2digit = pynini.string_file(
            get_abs_path("data/ordinals/superscript_digits.tsv")
        )
        ...
        self.superessive = self.bare_ordinals @ pynini.cdrewrite(
            superessive_endings, "", "[EOS]", NEMO_SIGMA
        )
        ...
        self.graph = pynini.union(
            self.filtered_ordinals + pynutil.delete("."),
            self.filtered_ordinals.project("output"),
        ).optimize()

        # For some reason, bare_ordinals does not work when exported, so doing this here
        fractional_exceptions = pynini.string_map(
            [("első", "egyed"), ("második", "fél")]
        )
        self.fractional = (
            bare_ordinals
            @ pynini.cdrewrite(
                fractional_exceptions, "[BOS]", "[EOS]", NEMO_SIGMA
            )
