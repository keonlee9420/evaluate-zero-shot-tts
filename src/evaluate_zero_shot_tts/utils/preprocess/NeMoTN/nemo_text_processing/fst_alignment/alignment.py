...
def parse_args():
    ...
    args.add_argument(
        "--fst", help="FAR file containing FST", type=str, required=True
    )
    args.add_argument(
        "--grammar",
        help="tn or itn",
    ...
    args.add_argument(
        "--start",
        help="start index of substring to be mapped",
        type=int,
        required=False,
    )
    args.add_argument(
        "--end",
        help="end index of substring to be mapped",
        type=int,
        required=False,
    )
...
def get_string_alignment(
    fst: pynini.Fst, input_text: str, symbol_table: pynini.SymbolTable
):
    """
    ...
    output = list(
        zip(
            [symbol_table.find(x) for x in ilabels],
            [symbol_table.find(x) for x in olabels],
        )
    )
...
def indexed_map_to_output(
    alignment: List[tuple], start: int, end: int, mode: str
):
    """
    ...
    logging.debug(
        f"0: |{list(map(remove, [x[0] for x in alignment[aligned_start:aligned_end+1]]))}|"
    )
    ...
    while (
        ...
        and (
            alignment[aligned_start - 1][1] in tn_itn_symbols
    ...
    while (
        ...
        and (
            alignment[aligned_end + 1][1] in tn_itn_symbols
            or alignment[aligned_end + 1][1] == EPS
        )
    ):
        aligned_end += 1
    ...
    output_og_start_index = _get_original_index(
        alignment=alignment, aligned_index=aligned_start
    )
    output_og_end_index = _get_original_index(
        alignment=alignment, aligned_index=aligned_end + 1
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise ValueError(
            f"{args.rule} not found. Please specify valid --rule argument."
        )
    input_text = args.text

    table = create_symbol_table()
    alignment, output_text = get_string_alignment(
        fst=fst, input_text=input_text, symbol_table=table
    )
    ...
    for x in indices:
        start, end = indexed_map_to_output(
            start=x[0], end=x[1], alignment=alignment, mode=args.grammar
        )
        logging.info(
            f"inp indices: [{x[0]}:{x[1]}] out indices: [{start}:{end}]"
        )
        logging.info(
            f"in: |{input_text[x[0]:x[1]]}| out: |{output_text[start:end]}|"
        )
