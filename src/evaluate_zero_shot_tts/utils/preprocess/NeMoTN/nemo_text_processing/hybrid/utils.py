...
cardinal_graph = (
    pynini.closure(pynini.union("In ", "in "))
    + cardinal_graph
    + pynini.closure(pynini.accep(" ") + cardinal_graph)
)
...
def load_data(input_fs: List[str]):
    ...
    targets = [
        [x for i, x in enumerate(sents) if ls[i]]
        for (sents, ls) in zip(sentences, labels)
    ]
    ...
    for i in range(len(inputs)):
        for target in targets[i]:
            ...
            for diff in diffs[::-1]:
                ...
                replacement = (
                    inputs[i][: diff[0][0]] + tg_diff + inputs[i][diff[0][1] :]
                )
                if (in_diff == "s" and tg_diff == "z") or (
                    in_diff == "z" and tg_diff == "s"
                ):
                    inputs[i] = replacement
                elif (in_diff == "re" and tg_diff == "er") or (
                    in_diff == "er" and tg_diff == "re"
                ):
                    inputs[i] = replacement
                elif (in_diff == "me" and tg_diff == "") or (
                    in_diff == "" and tg_diff == "me"
                ):
                    inputs[i] = replacement
                elif (in_diff == "ue" and tg_diff == "") or (
                    in_diff == "" and tg_diff == "ue"
                ):
                    inputs[i] = replacement
        ...
        for target in targets[i]:
            ...
            for diff in diffs[::-1]:
                ...
                if (in_diff == "s" and tg_diff == "z") or (
                    ...
                    inputs[i] = replacement
                elif (in_diff == "" and tg_diff == "u") or (
                    in_diff == "u" and tg_diff == ""
                ):
                    inputs[i] = replacement
                elif (in_diff == "ue" and tg_diff == "") or (
                    in_diff == "" and tg_diff == "ue"
...
def clean_pre_norm(
    inputs: List[str], targets: List[List[str]], dataset: Optional[str] = None
):
    """
    standardizes format of inputs and targets before being normalized, so more rules apply.
    """
    ...
    if dataset == "libritts":
        pre_inputs, pre_targets = _clean_pre_norm_libritts(
            inputs=pre_inputs, targets=pre_targets
        )
    elif dataset == "google":
        pre_inputs, pre_targets = _clean_pre_norm_google(
            inputs=pre_inputs, targets=pre_targets
        )
    else:
        pass
    ...
    for i in range(len(pre_inputs)):
        pre_inputs[i] = re.sub(
            "librivox.org", "librivox dot org", pre_inputs[i]
        )
        pre_inputs[i] = re.sub(
            rf"([0-9]?[0-9](\.|:)[0-9][0-9]\s?)(a|A|p|P)(\.?)\s(M|m)(\.?)",
            rf"\1\3\4\5\6",
            pre_inputs[i],
        )
...
def _clean_post_norm_libritts(
    inputs: List[str], targets: List[List[str]], norm_texts
):
    return targets, norm_texts


def _clean_post_norm_google(
    inputs: List[str], targets: List[List[str]], norm_texts
):
    """
        ...
        for target in targets[i]:
            for j, norm in enumerate(norm_texts[i][0]):
                ...
                for diff in diffs[::-1]:
                    ...
                    tg_diff = target[diff[1][0] : diff[1][1]].lower()
                    replacement = (
                        norm[: diff[0][0]] + tg_diff + norm[diff[0][1] :]
                    )
...
def adjust_pred(pred: str, gt: str, dataset: str, delim_present=True):
    ...
    if dataset == "libritts" and pred != gt:
        if "dollar" in gt:
            ...
            if re.search(r"\bus dollar", pred) and not re.search(
                r"\bus dollar", gt
            ):
    ...
    if gt != pd and dataset in ["google", "libritts"]:
        if gt.replace("/", "").replace("  ", " ") == pred.replace(
            "slash", ""
        ).replace("  ", " "):
            pred = gt
        ...
        #     pred = gt
        elif gt.replace("to", "").replace("-", "") == pred.replace(
            "to", ""
        ).replace("-", ""):
            pred = gt
            ...
            for jj, option in enumerate(post_norm_texts[i][0]):
                for _, _target in enumerate(_targets):
                    if not delim_present:
                        ...
                        _target = remove_punctuation(
                            _target, remove_spaces=False, do_lower=True
                        )
                        option = remove_punctuation(
                            option, remove_spaces=False, do_lower=True
                        )
                    ...
                    for diff in diffs[::-1]:
                        ...
                        logging.debug(
                            f"|{post_norm_texts[i][0][jj]}| -> |{new_pred}|"
                        )
...
def clean_libri_tts(target: str):
    ...
    for abbr in google_abbr2expand.keys():
        if abbr in target:
            ...
            target = re.sub(
                rf"(^|\s|\W){abbr}($|\s)",
                rf"\1{google_abbr2expand[abbr]}\2",
                target,
            )
...
def remove_punctuation(
    text: str, remove_spaces=True, do_lower=True, lang="en", exclude=None
):
...
def get_alternative_label(pred: str, targets: List[str]) -> bool:
    ...
    for target in targets:
        target = _clean_post_general(
            remove_punctuation(target, remove_spaces=False, do_lower=True)
        )
        target = _clean_targets(
            remove_punctuation(target, remove_spaces=False, do_lower=True)
        )
...
def is_date(pred, gt, cardinal_graph):
    ...
    if (
        "thousand" in pred
        and "hundred" in pred
        and pred.strip().split()[-2:] == gt.strip().split()[-2:]
    ):
        is_date_case = True
    elif (
        "thousand" in gt
        and "hundred" in gt
        and gt.strip().split()[-2:] == pred.strip().split()[-2:]
    ):
        is_date_case = True
    else:
        try:
            if top_rewrite(
                gt.replace(" oh ", " zero ").replace(" o ", " zero "),
                cardinal_graph,
            ).replace(" ", "") == top_rewrite(
                pred.replace(" oh ", " zero ").replace(" o ", " zero "),
                cardinal_graph,
            ).replace(
                " ", ""
            ):
...
def is_correct(pred: str, targets: Union[List[str], str], lang: str) -> bool:
    ...
    if isinstance(targets, List):
        targets = [
            remove_punctuation(x, remove_spaces=True, do_lower=True, lang=lang)
            for x in targets
        ]
    else:
        targets = [
            remove_punctuation(
                targets, remove_spaces=True, do_lower=True, lang=lang
            )
        ]
...
def diff_pred_gt(pred: str, gt: str):
    ...
    for i, seq in enumerate(s):
        ...
        if (
            right[idx - 1] <= seq[0] < left[idx]
            and (seq[0] + seq[2]) <= left[idx]
        ):
            matches.append(seq)
