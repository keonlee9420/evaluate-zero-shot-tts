...
parser.add_argument(
    "--n_tagged", default=100, type=int, help="Number WFST options"
)
parser.add_argument(
    "--context_len",
    default=-1,
    type=int,
    help="Context length, -1 to use full context",
)
parser.add_argument(
    "--threshold", default=0.2, type=float, help="delta threshold value"
)
parser.add_argument(
    "--overwrite_cache", action="store_true", help="overwrite cache"
)
parser.add_argument("--model_name", type=str, default="bert-base-uncased")
parser.add_argument(
    "--cache_dir", default="cache", type=str, help="use cache dir"
)
parser.add_argument(
    "--data",
...
parser.add_argument(
    "--models",
    default="mlm_bert-base-uncased",
    type=str,
    help="Comma separated string of model names",
)
parser.add_argument(
    "--regenerate_pkl",
    action="store_true",
...
def _get_unchanged_count(text):
    ...
    text = utils.remove_punctuation(
        text, remove_spaces=False, do_lower=False, exclude=exclude
    )
    text_clean = ""
    for ch in text:
...
def main():
    ...
    pre_inputs, pre_targets = utils.clean_pre_norm(
        dataset=args.dataset, inputs=inputs, targets=targets
    )
    ...
    if not os.path.exists(p_file) or args.regenerate_pkl:
        ...
        def __process_batch(batch_idx, batch, dir_name):
            ...
            with open(f"{dir_name}/{batch_idx}.p", "wb") as handle:
                pickle.dump(
                    normalized, handle, protocol=pickle.HIGHEST_PROTOCOL
                )
        ...
        with open(p_file, "wb") as handle:
            pickle.dump(
                norm_texts_weights, handle, protocol=pickle.HIGHEST_PROTOCOL
            )
    else:
    ...
    if args.threshold > 0:
        norm_texts_weights = threshold_weights(
            norm_texts_weights, delta=args.threshold
        )
    ...
    post_targets, post_norm_texts_weights = utils.clean_post_norm(
        ...
        inputs=pre_inputs,
        targets=pre_targets,
        norm_texts=norm_texts_weights,
    )

    print("GETTING LABELS...")
    labels = utils.get_labels(
        targets=post_targets, norm_texts_weights=post_norm_texts_weights
    )

    examples_with_no_labels_among_wfst = [
        i for i, x in enumerate(labels) if 1 not in x
    ]
    ...
    for i, example in tqdm(enumerate(zip(post_norm_texts_weights, labels))):
        ...
        df["sent"] = df["sent"].apply(
            lambda x: utils.remove_whitelist_boudaries(x)
        )
        df["weights"] = data[1]
        ...
        for model in models:
            ...
            pred_is_correct = min(
                sum((df["labels"] == df[f"{model}_pred"]) & df["labels"] == 1),
                1,
            )
    ...
    print(
        f"examples_with_no_labels_among_wfst: {len(examples_with_no_labels_among_wfst)}"
    )
    return all_correct
