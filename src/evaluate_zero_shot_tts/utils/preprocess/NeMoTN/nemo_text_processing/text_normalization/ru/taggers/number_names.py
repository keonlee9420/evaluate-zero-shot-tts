...
from nemo_text_processing.text_normalization.ru.utils import (
    get_abs_path,
    load_labels,
)
...
def get_number_names():
    ...
    for case in [
        ...
        cardinal_l |= (
            pynini.closure(case + pynini.accep(" ")) + case
        ).optimize()
    ...
    fg_ordinal = (
        pynutil.add_weight(pynini.compose(fg, complex_numbers), -1) | fg
    )
    ...
    number_names["ordinal_number_names"] = (
        fg_ordinal @ (p @ ordinal_l)
    ).optimize()
    number_names["cardinal_number_names"] = (fg @ (p @ cardinal_l)).optimize()
    number_names["cardinal_names_nominative"] = (
        fg @ (p @ cardinal_names_nominative_l)
    ).optimize()
    return number_names
...
def get_alternative_formats():
    ...
    one_thousand_alternative = pynini.cdrewrite(
        one_thousand_map, "[BOS]", "", NEMO_SIGMA
    )
    ...
    ] = one_thousand_alternative.optimize()
