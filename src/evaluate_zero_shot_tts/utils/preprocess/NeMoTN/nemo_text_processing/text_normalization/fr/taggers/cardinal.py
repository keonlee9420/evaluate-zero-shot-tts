...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        teens = pynini.string_file(
            get_abs_path("data/numbers/teens.tsv")
        )  # 11 to 19
        ...
        ties_simple = tens + (
            pynutil.delete("0") | (ties_simple_unit | ties_simple_one)
        )  # 20 to 69

        # Complex tens
        seventy = pynini.string_file(get_abs_path("data/numbers/seventy.tsv"))
        seventy_unit = pynutil.insert("-") + (
            (pynutil.insert("1") + NEMO_DIGIT) @ teens_no_one
        )
        seventy_one = insert_space + pynini.cross("1", "et onze")
        seventies = seventy + (
            pynini.cross("0", "-dix") | (seventy_unit | seventy_one)
        )  # 70 to 79

        eighty = pynini.string_file(get_abs_path("data/numbers/eighty.tsv"))
        eighties = eighty + (
            pynini.cross("0", "s") | (pynutil.insert("-") + digits)
        )  # 80 to 89

        ninety = pynini.string_file(get_abs_path("data/numbers/ninety.tsv"))
        nineties_unit = pynutil.insert("-") + (
            (pynutil.insert("1") + NEMO_DIGIT) @ teens
        )
        nineties = ninety + (
            pynini.cross("0", "-dix") | nineties_unit
        )  # 90 to 99

        all_double_digits = (
            ten | teens | ties_simple | seventies | eighties | nineties
        )
        ...
        # Hundreds
        hundreds_parts = (pynutil.delete("0") + insert_space + digits) | (
            insert_space + all_double_digits
        )
        one_hundreds = pynini.cross("1", "cent") + (
            pynutil.delete("00") | hundreds_parts
        )
        multiple_hundreds = (
            digits_no_one + insert_space + pynutil.insert("cent")
        ...
        all_thousands = (
            ...
            | (
                ...
                + pynutil.delete("00")
                + insert_space
                + (pynini.cross("1", "et un") | digits_no_one)
            )
            | (other_thousands + pynutil.delete("00") + insert_space + digits)
            | (
                (one_thousand | other_thousands)
                + pynutil.delete("0")
                + insert_space
                + all_double_digits
            )
        ...
        # Millions
        digits_to_hundreds_no_one = (
            digits_no_one | all_double_digits | all_hundreds
        )
        digits_to_thousands_no_one = (
            digits_no_one | all_double_digits | all_hundreds | all_thousands
        )
        millions_components = pynini.closure(
            delete_separator + pynini.closure(NEMO_DIGIT, 3), 2
        )
        ...
        all_millions = (
            ...
            @ (
                ...
                + pynini.closure(
                    insert_space
                    + (digits_to_thousands_no_one | pynini.cross("1", "et un")),
                    0,
                    1,
                )
            )
        ...
        billions_components = pynini.closure(
        ...
        all_billions = (
            ...
            @ (
                ...
                + pynini.closure(
                    insert_space
                    + (digits_to_millions_no_one | pynini.cross("1", "et un")),
                    0,
                    1,
                )
            )
        ...
        final_graph_fem = final_graph_masc | (
            final_graph_masc
            @ (pynini.closure(NEMO_SIGMA) + pynini.cross("un", "une"))
        )
