...
class FractionFst(GraphFst):
    ...
    def __init__(self, ordinal: GraphFst, deterministic: bool = True):
        ...
        irregular_denominators = pynini.string_file(
            get_abs_path("data/fractions/irregular_denominators.tsv")
        )
        denominator = pynini.union(
            pynutil.add_weight(denominator @ irregular_denominators, -0.01),
            denominator,
        )
        denominator = (
            pynini.cross("denominator:", "integer:")
            + pynini.closure(NEMO_SIGMA)
        ) @ denominator

        irregular_half = pynini.closure(NEMO_SIGMA) + pynini.cross(
            "et un demi", "et demi"
        )
        ...
        optional_sign = pynini.closure(
            pynini.cross('negative: "true" ', "moins") + insert_space, 0, 1
        )
        ...
        final_graph = pynini.union(
            pynutil.add_weight(final_graph @ irregular_half, -0.01), final_graph
        )
