...
class DecimalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        decimal_number = (
            digit_no_zero @ cardinal_graph
            + final_zero
            + pynutil.insert(" tized")
        )
        decimal_number |= (
            (digit_or_del_zero + NEMO_DIGIT) @ cardinal_graph
            + final_zero
            + pynutil.insert(" század")
        )
        order = 2
        for decimal_name in [
            "ezred",
        ...
        if not deterministic:
            alts = pynini.string_map(
                [
                    ("billiomod", "ezer milliárdod"),
                    ("billiárdod", "millió milliárdod"),
                ]
            )
            decimal_alts = decimal_number @ pynini.cdrewrite(
                alts, "", "[EOS]", NEMO_SIGMA
            )
        ...
        final_graph_wo_sign = (
            self.graph_integer + point + insert_space + self.graph_fractional
        )
