...
def make_million(word: str, hundreds: "pynini.FstLike", deterministic=False):
    ...
    if not deterministic:
        graph_million |= pynutil.add_weight(
            pynini.cross("001", "egy{word}"), -0.001
        )
        graph_million |= pynutil.add_weight(
            pynini.cross("001", "egy{word} "), -0.001
        )
        graph_million |= pynutil.add_weight(
            pynini.cross("001", "{word} "), -0.001
        )
        graph_million |= pynutil.add_weight(
            pynini.cross("001", " egy{word}"), -0.001
        )
...
class CardinalFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        zero = pynini.invert(
            pynini.string_file(get_abs_path("data/number/zero.tsv"))
        )
        digit = pynini.invert(
            pynini.string_file(get_abs_path("data/number/digit.tsv"))
        )
        digit_inline = pynini.invert(
            pynini.string_file(get_abs_path("data/number/digit_inline.tsv"))
        )
        tens = pynini.invert(
            pynini.string_file(get_abs_path("data/number/tens.tsv"))
        )
        tens_inline = pynini.invert(
            pynini.string_file(get_abs_path("data/number/tens_inline.tsv"))
        )
        ...
        clean_output = (
            pynini.cdrewrite(
                delete_space | delete_hyphen, "[BOS]", "", NEMO_SIGMA
            )
            @ pynini.cdrewrite(
                delete_space | delete_hyphen, "", "[EOS]", NEMO_SIGMA
            )
            @ pynini.cdrewrite(
                delete_extra_hyphens | delete_extra_spaces, "", "", NEMO_SIGMA
            )
            @ pynini.cdrewrite(
        ...
        self.four_digits_read = pynini.union(
            ((NEMO_DIGIT - "0") + (NEMO_DIGIT**3)) @ self.graph,
