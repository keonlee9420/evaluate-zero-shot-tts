...
def parse_args():
    ...
    parser.add_argument(
        "--filter",
        action="store_true",
        help="clean data for normalization purposes",
    )
...
if __name__ == "__main__":
    ...
    if args.category is None:
        ...
        (
            ...
            _,
        ) = training_data_to_sentences(training_data)
        print("- Data: " + str(len(sentences_normalized)) + " sentences")
        sentences_prediction = normalizer.normalize_list(
            sentences_un_normalized
        )
    ...
    tokens_per_type = training_data_to_tokens(
        training_data, category=args.category
    )
    token_accuracy = {}
    for token_type in tokens_per_type:
        ...
        token_accuracy[token_type] = evaluate(
            preds=tokens_prediction,
            labels=tokens_normalized,
            input=tokens_un_normalized,
        )
        print("  - Accuracy: " + str(token_accuracy[token_type]))
    token_count_per_type = {
        token_type: len(tokens_per_type[token_type][0])
        for token_type in tokens_per_type
    }
    ...
    print(
        "- Accuracy: "
        + str(sum(token_weighted_accuracy) / sum(token_count_per_type.values()))
    )
    ...
    if args.category is None:
        ...
        c2 = ["Num Tokens", len(sentences_normalized)] + [
            token_count_per_type[known_type]
            if known_type in tokens_per_type
            else "0"
