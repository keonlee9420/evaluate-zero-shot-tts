...
gender_suffix = pynini.string_file(
    get_abs_path("data/ordinals/gender_suffix.tsv")
)
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        graph_tens_component = (
            ...
            | (
                graph_ties
                + pynini.closure(
                    pynini.cross(" y ", NEMO_SPACE) + graph_digit, 0, 1
                )
            )
            | graph_twenties
        )
        ...
        graph_hundred_component = pynini.union(
            graph_hundreds
            + pynini.closure(
                NEMO_SPACE + pynini.union(graph_tens_component, graph_digit),
                0,
                1,
            ),
            graph_tens_component,
        ...
        graph_thousands @= pynini.cdrewrite(
            delete_space, "", "milésimo", NEMO_SIGMA
        )  # merge as a prefix
        graph_thousands |= thousands

        self.multiples_of_thousand = (
            cardinal_graph @ graph_thousands
        ).optimize()
        ...
        if (
            ...
            graph_thousands |= (
                (self.one_to_one_thousand @ graph_hundred_component)
                + NEMO_SPACE
                + thousands
            )

        graph_thousands += pynini.closure(
            NEMO_SPACE + graph_hundred_component, 0, 1
        )
        ...
        if not deterministic:
            ...
            split_words = pynini.cross("decimo", "décimo ") | pynini.cross(
                "vigesimo", "vigésimo "
            )
            split_words = pynini.cdrewrite(
                split_words, "", NEMO_CHAR, NEMO_SIGMA
            )
            ordinal_graph |= ordinal_graph @ split_words

        # If "octavo" is preceeded by the "o" within string, it needs deletion
        ordinal_graph @= pynini.cdrewrite(
            pynutil.delete("o"), "", "octavo", NEMO_SIGMA
        )
        ...
        delete_period = pynini.closure(
            pynutil.delete("."), 0, 1
        )  # Sometimes the period is omitted
        ...
        exceptions = pynini.string_file(
            get_abs_path("data/ordinals/roman_exceptions.tsv")
        )
        graph_exception = pynini.project(exceptions, "input")
        roman_ordinals = (
            pynini.project(roman_ordinals, "input") - graph_exception.arcsort()
        ) @ roman_ordinals

        graph_roman = (
            pynutil.insert('integer: "') + roman_ordinals + pynutil.insert('"')
        )
