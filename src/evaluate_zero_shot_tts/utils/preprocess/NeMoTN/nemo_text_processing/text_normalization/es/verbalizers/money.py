...
class MoneyFst(GraphFst):
    ...
    def __init__(self, decimal: GraphFst, deterministic: bool = True):
        super().__init__(
            name="money", kind="verbalize", deterministic=deterministic
        )
        ...
        maj_masc = (
            maj_plural_masc | maj_singular_masc
        )  # Tagger kept quantity resolution stable
        ...
        graph_integer_fem = (
            shift_cardinal_gender(integer_part) + NEMO_SPACE + maj_fem
        )
        ...
        graph_integer_with_minor_masc = (
            ...
            + pynini.union(
                ...
                (
                    optional_add_and
                    + shift_cardinal_gender(fractional_part)
                    + NEMO_SPACE
                    + min_fem
                ),
        ...
        graph_integer_with_minor = (
            graph_integer_with_minor_fem | graph_integer_with_minor_masc
        )
        ...
        graph_decimal = (
            pynini.cdrewrite(
                pynutil.insert(" de"),
                'quantity: "' + pynini.closure(NEMO_NOT_QUOTE, 1),
                '"',
                NEMO_SIGMA,
            )
            @ graph_decimal
        ...
        # *** current_min
        graph_minor_masc = (
            fractional_part + NEMO_SPACE + min_masc + delete_preserve_order
        )
        graph_minor_fem = (
            shift_cardinal_gender(fractional_part)
            + NEMO_SPACE
