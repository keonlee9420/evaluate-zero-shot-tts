...
class TelephoneFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        single_digits @= pynini.cdrewrite(
            pynini.cross(ones, "uno"), "", "", NEMO_SIGMA
        )
        ...
        double_digits = teen
        double_digits |= ties + (
            pynutil.delete("0") | (pynutil.insert(" y ") + single_digits)
        )
        ...
        tel_prompt_sequence = (
            telephone_prompts + NEMO_SPACE + pynini.closure(country_code, 0, 1)
        )
        ...
        digit_to_str_graph = single_digits + pynini.closure(
            pynutil.insert(" ") + single_digits, 0, 2
        )
        ip_graph = (
            digit_to_str_graph
            + (pynini.cross(".", " punto ") + digit_to_str_graph) ** 3
        )
        ...
        num_letter_area_code = area_code @ pynini.cross(
            "ocho cero cero ", "ochocientos "
        )
        number_word = pynini.closure(single_digits | NEMO_ALPHA, 1, 8)
        number_words = (
            pynini.closure(number_word + pynini.cross(separators, " "), 0, 2)
            + number_word
        )

        nums_w_letters_graph = pynutil.add_weight(
            num_letter_area_code + delete_separator + number_words, 0.01
        )
        ...
        graph = (
            pynini.closure(
                country_code_graph + delete_separator + insert_space, 0, 1
            )
            + number_part
