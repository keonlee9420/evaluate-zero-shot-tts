...
class NormalizerWithAudio(Normalizer):
        ...
        **kwargs,
        ...
        try:
            ...
            raise RecursionError(
                f"RecursionError. Try decreasing --max_number_of_permutations_per_split"
            )
        ...
        (
            ...
            pred_text_spans,
            norm_spans,
            text_with_span_tags_list,
            masked_idx_list,
        ) = get_alignment(text, det_norm, pred_text, verbose=False)

        sem_tag_idx = 0
        for cur_semiotic_span, cur_pred_text, cur_deter_norm in zip(
            semiotic_spans, pred_text_spans, norm_spans
        ):
    ...
    def normalize_non_deterministic(
            ...
            if self.lang == "en":
                ...
                if "[" in text and "]" in text:
                    lattice = rewrite.rewrite_lattice(
                        text, self.tagger_non_deterministic.fst
                    )
                else:
                    try:
                        lattice = rewrite.rewrite_lattice(
                            text, self.tagger_non_deterministic.fst_no_digits
                        )
                ...
                tagged_texts = [
                    (x[1], float(x[2])) for x in lattice.paths().items()
                ]
        ...
        if self.lang == "en":
            ...
            normalized_texts = [
                self.post_process(text) for text in normalized_texts
            ]
        else:
            normalized_texts = []
            for tagged_text in tagged_texts:
                self._verbalize(
                    tagged_text, normalized_texts, n_tagged, verbose=verbose
                )
        ...
        if punct_post_process:
            ...
            if self.moses_detokenizer:
                normalized_texts = [
                    self.moses_detokenizer.detokenize([t])
        ...
        if self.lm:
            remove_dup = sorted(
                list(set(zip(normalized_texts, weights))), key=lambda x: x[1]
            )
            ...
            if self.lang == "en":
                ...
                if "[" in text and "]" in text:
                    tagged_texts = rewrite.top_rewrites(
                        text,
                        self.tagger_non_deterministic.fst,
                        nshortest=n_tagged,
                    )
                else:
                    try:
                        ...
                        tagged_texts = rewrite.top_rewrites(
                            text,
                            self.tagger_non_deterministic.fst_no_digits,
                            nshortest=n_tagged,
                        )
                    except pynini.lib.rewrite.Error:
                        tagged_texts = rewrite.top_rewrites(
    ...
    def _verbalize(
        ...
        def get_verbalized_text(tagged_text):
            return rewrite.top_rewrites(
                tagged_text, self.verbalizer_non_deterministic.fst, n_tagged
            )
    ...
    def select_best_match(
        ...
        """
        normalized_texts_cer = calculate_cer(
            normalized_texts, pred_text, remove_punct
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--text",
        help="input string or path to a .txt file",
        default=None,
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=["en", "ru", "de", "es", "sv"],
        default="en",
        type=str,
    )
    parser.add_argument(
        "--manifest", default=None, help="path to .json manifest"
    )
    parser.add_argument(
        "--output_filename",
        default=None,
