...
class DecimalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool):
        ...
        cardinal_numbers_with_leading_zeros = (
        ...
        graph_fractional = (
            NEMO_DIGIT @ cardinal_numbers_with_leading_zeros
            + add_preposition
            + pynutil.insert("عشرة")
        )
        graph_fractional |= (
            (NEMO_DIGIT + NEMO_DIGIT) @ cardinal_numbers_with_leading_zeros
            + add_preposition
            + pynutil.insert("مئة")
        )
        graph_fractional |= (
        ...
        self.optional_quantity = pynini.string_file(
            get_abs_path("data/number/quantities.tsv")
        ).optimize()
        ...
        self.graph_decimal = (
            self.integer_part + insert_space + self.seperator + graph_fractional
        )

        self.final_graph_decimal = (
            cardinal.optional_minus_graph
            + graph_integer_or_none
            + insert_space
            + graph_fractional
            + optional_quantity
        )
