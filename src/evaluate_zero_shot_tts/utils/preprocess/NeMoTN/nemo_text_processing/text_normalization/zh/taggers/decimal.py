...
def get_quantity(decimal):
    ...
    res = (
        decimal + pynutil.insert(' quantity: "') + suffix + pynutil.insert('"')
    )
...
class DecimalFst(GraphFst):
    ...
    def __init__(
        self, cardinal: GraphFst, deterministic: bool = True, lm: bool = False
    ):
        super().__init__(
        ...
        ) | pynini.closure(pynini.cross("0", "零"))
        ...
        graph_fraction = (
            ...
            + pynini.closure(cardinal_after_decimal, 1)
            + pynutil.insert('"')
        )
        graph_decimal = (
            graph_integer + decimal_point + pynutil.insert(" ") + graph_fraction
        )
