    ...
    def __init__(
        ...
        if remove_puncts:
            remove_puncts_graph = pynutil.delete(
                pynini.union(
                    NEMO_PUNCT,
                    pynini.string_file(
                        get_abs_path("data/char/punctuations_zh.tsv")
                    ),
                )
            )
            graph @= pynini.cdrewrite(
                remove_puncts_graph, "", "", NEMO_SIGMA
            ).optimize()
        ...
        if to_upper or to_lower:
            if to_upper:
                ...
                conv_cases_graph = pynini.string_file(
                    get_abs_path("data/char/upper_to_lower.tsv")
                )

            graph @= pynini.cdrewrite(
                conv_cases_graph, "", "", NEMO_SIGMA
            ).optimize()
        ...
        if tag_oov:
            ...
            zh_charset_ext = pynini.string_file(
                get_abs_path("data/char/charset_extension.tsv")
            )
