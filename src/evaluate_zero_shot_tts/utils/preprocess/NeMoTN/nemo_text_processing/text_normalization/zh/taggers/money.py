...
class MoneyFst(GraphFst):
    ...
    def __init__(
        ...
        currency = pynini.string_file(
            get_abs_path("data/money/currency_major.tsv")
        )
        currency_mandarin = pynini.string_file(
            get_abs_path("data/money/currency_mandarin.tsv")
        )

        # regular money gramamr with currency symbols $1000
        currency_component = (
            pynutil.insert('currency: "') + currency + pynutil.insert('"')
        )
        number_component = (
            pynutil.insert('integer: "') + cardinal + pynutil.insert('"')
        )
        graph_regular_money = (
            currency_component + pynutil.insert(" ") + number_component
        )
        ...
        graph_unit_only = (
            ...
            + unit_components
            + pynini.closure(
                pynutil.insert(" ") + currency_mandarin_component, 0, 1
            )
        )
        graph_units = pynini.closure(graph_unit_only, 1, 3)

        # only currency part as mandarins
        graph_mandarin_money = (
            number_component + pynutil.insert(" ") + currency_mandarin_component
        )

        # larger money as decimals
        graph_decimal_money = (
            decimal + pynutil.insert(" ") + currency_mandarin_component
