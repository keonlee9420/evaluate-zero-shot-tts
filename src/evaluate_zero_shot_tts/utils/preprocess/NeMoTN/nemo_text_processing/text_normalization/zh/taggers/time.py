...
class TimeFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True, lm: bool = False):
        ...
        hour_minute = (
            hour_component + symbol + pynutil.insert(" ") + minute_component
        )
        ...
        minute_duration = (
            pynini.accep("分钟") | pynini.accep("刻") | pynini.accep("刻钟")
        )
        second_duration = (
            pynini.accep("秒钟") | pynini.cross("秒鐘", "秒钟") | pynini.accep("秒")
        )
        # combining two above
        hour_component = (
            pynutil.insert('hour: "')
            + hour
            + (hour_clock | hour_duration)
            + pynutil.insert('"')
        )
        minute_component = (
            pynutil.insert('minute: "')
            + minute
            + (minute_clock | minute_duration)
            + pynutil.insert('"')
        )
        second_component = (
            pynutil.insert('second: "')
            + second
            + (second_clock | second_duration)
            + pynutil.insert('"')
        )
        ...
        backcount = (
            pynutil.insert('verb: "') + pynini.accep("差") + pynutil.insert('"')
        )
        graph_hour = (
            (
        ...
        am_component = (
            pynutil.insert('suffix: "') + suffix_am + pynutil.insert('"')
        )
        pm_component = (
            pynutil.insert('suffix: "') + suffix_pm + pynutil.insert('"')
        )

        graph_suffix = (
            (graph_clock_period | graph_colon | graph_backcount)
            + pynutil.insert(" ")
            + (am_component | pm_component)
        )

        graph = (
            graph_colon | graph_clock_period | graph_backcount | graph_suffix
        )
