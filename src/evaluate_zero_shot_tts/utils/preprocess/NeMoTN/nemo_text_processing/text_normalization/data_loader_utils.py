    ...
    with open(file_path, "r") as fp:
        for line in fp:
            ...
            if parts[0] == "<eos>":
                res.append(
                    Instance(
                        token_type=EOS_TYPE, un_normalized="", normalized=""
                    )
                )
            else:
...
def training_data_to_sentences(
    data: List[Instance],
    ...
    un_normalized = [
        " ".join([instance.un_normalized for instance in sentence])
        for sentence in sentences
    ]
    normalized = [
        " ".join([instance.normalized for instance in sentence])
        for sentence in sentences
    ]
...
def post_process_punct(
    input: str, normalized_text: str, add_unicode_punct: bool = False
):
    """
    ...
    for punct in punct_marks:
        try:
            ...
            while punct in input[idx_in:]:
                ...
                if not equal and not _is_valid(
                    idx_out, idx_in, normalized_text, input
                ):
                    idx_in += 1
                    continue
                ...
                if (
                    idx_in < len(input) - 1
                    and idx_out < len(normalized_text) - 1
                ):
                    if (
                        normalized_text[idx_out + 1] == " "
                        and input[idx_in + 1] != " "
            ...
            logging.info(
                f"Skipping post-processing of {''.join(normalized_text)} for '{punct}'"
            )
