...
class FractionFst(GraphFst):
    ...
    def __init__(self, ordinal: GraphFst, deterministic: bool = True):
        ...
        change_numerator_two = pynini.cdrewrite(
            pynini.cross("zweitel", "halbe"), "[BOS]", "[EOS]", NEMO_SIGMA
        )
        integer = (
        ...
        numerator = (
            pynutil.delete('numerator: "') + change_one + pynutil.delete('" ')
        )
        denominator = (
            ...
            @ (
                pynini.cdrewrite(
                    pynini.closure(ordinal.ordinal_stem, 0, 1),
                    "",
                    "[EOS]",
                    NEMO_SIGMA,
                )
        ...
        integer += insert_space + pynini.closure(
            pynutil.insert("und ", weight=0.001), 0, 1
        )

        denominator_one_half = pynini.cdrewrite(
            pynini.cross("ein halbe", "ein halb"), "[BOS]", "[EOS]", NEMO_SIGMA
        )

        fraction_default = (
            numerator + insert_space + denominator
        ) @ denominator_one_half

        self.graph = (
            optional_sign + pynini.closure(integer, 0, 1) + fraction_default
        )
