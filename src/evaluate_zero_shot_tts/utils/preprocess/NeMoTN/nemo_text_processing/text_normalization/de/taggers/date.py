...
def get_year_graph(cardinal: GraphFst) -> "pynini.FstLike":
    ...
    **00 ** hundert
    ...
    # for 20**
    graph_double_double |= (
        pynini.accep("20") @ graph_two_digit + insert_space + graph_two_digit
    )
    graph = (
        graph_double_double
        | (pynini.accep("1") + NEMO_DIGIT) @ graph_two_digit
...
class DateFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool):
        ...
        year = pynutil.add_weight(
            get_year_graph(cardinal=cardinal), weight=0.001
        )
        self.year = year
        ...
        for sep in separators:
            ...
            new_graph = (
                day + pynini.cross(sep, " ") + month_number + year_optional
            )
            graph_dmy |= new_graph
        ...
        graph_ymd = (
            year_only + pynini.cross(dash, " ") + month_number + day_optional
        )
