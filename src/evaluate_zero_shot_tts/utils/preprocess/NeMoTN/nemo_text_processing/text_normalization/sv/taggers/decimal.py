...
def get_quantity(
    ...
    quantities_abbr = pynini.string_file(
        get_abs_path("data/numbers/millions_abbr.tsv")
    )

    quantities_pl = quantities + "er"
    quantities_pl |= quantities @ pynini.cdrewrite(
        pynini.cross("", "er"), "", "[EOS]", NEMO_SIGMA
    )
...
class DecimalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool):
        ...
        self.cardinal_graph_hundreds_one_non_zero_en = (
            cardinal_graph_hundreds_one_non_zero_en
        )
        self.cardinal_graph_hundreds_one_non_zero = (
            cardinal_graph_hundreds_one_non_zero
        )

        self.graph = cardinal.two_or_three_digits_read_frac

        self.graph_itn = pynini.invert(
            cardinal.two_or_three_digits_read_frac_both
        ).optimize()
        ...
        self.final_graph_wo_negative_w_abbr = (
            final_graph_wo_sign | quantity_w_abbr
        )
        self.final_graph_wo_negative_w_abbr_en = (
            final_graph_wo_sign_en | quantity_w_abbr
        )
        self.final_graph_wo_negative = final_graph_wo_sign | quantity_wo_abbr
        self.final_graph_wo_negative_en = (
            final_graph_wo_sign_en | quantity_wo_abbr
        )

        final_graph = (
            optional_graph_negative + self.final_graph_wo_negative_w_abbr
        )
