...
class ElectronicFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        accepted_characters_with_dot = pynini.closure(
            NEMO_ALPHA | NEMO_DIGIT | accepted_symbols | dot
        )
        ...
        domain_graph = (
            pynutil.insert('domain: "') + domain_graph + pynutil.insert('"')
        )
        domain_common_graph = (
        ...
        protocol_end = (
            pynini.accep("www.")
            if deterministic
            else pynini.accep("www.")
            | pynini.cross("www.", "dubbel ve dubbel ve dubbel ve.")
        )
        protocol = (
            protocol_start | protocol_end | (protocol_start + protocol_end)
        )
        protocol = (
            pynutil.insert('protocol: "') + protocol + pynutil.insert('"')
        )
