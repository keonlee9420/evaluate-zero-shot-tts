...
class FractionFst(GraphFst):
    ...
    def __init__(
        ...
        alt_fractional_endings = pynini.string_map(
            [("tondel", "tondedel"), ("tiondel", "tiondedel")]
        )
        lexicalised = pynini.string_map(
            [("andradel", "halv"), ("fjärdedel", "kvart")]
        )
        alt_lexicalised = pynini.string_map(
            [("halv", "andradel"), ("kvart", "fjärdedel"), ("kvart", "kvarts")]
        )
        ...
        fractional_pl_endings = pynini.string_map(
            [("kvart", "kvartar"), ("halv", "halva"), ("del", "delar")]
        )
        fractions_pl = fractions @ pynini.cdrewrite(
            fractional_pl_endings, "", "[EOS]", NEMO_SIGMA
        )
        ...
        graph |= pynini.closure(integer + ensure_space, 0, 1) + pynini.compose(
            pynini.string_file(get_abs_path("data/numbers/fraction.tsv")),
            (numerator + denominator),
        )
