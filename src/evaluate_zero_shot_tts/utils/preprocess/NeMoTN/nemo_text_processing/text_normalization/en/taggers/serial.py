...
class SerialFst(GraphFst):
    ...
    def __init__(
        ...
        super().__init__(
            name="integer", kind="classify", deterministic=deterministic
        )
        ...
        if deterministic:
            num_graph = pynini.compose(
                NEMO_DIGIT ** (6, ...), cardinal.single_digits_graph
            ).optimize()
            num_graph |= pynini.compose(
                NEMO_DIGIT ** (1, 5), cardinal.graph
            ).optimize()
        ...
        if not self.deterministic and not lm:
            ...
            num_graph |= pynini.compose(
                num_graph, NEMO_SIGMA + pynutil.delete("hundred ") + NEMO_SIGMA
            )
        ...
        symbols = [
            x[0] for x in load_labels(get_abs_path("data/whitelist/symbol.tsv"))
        ]
        ...
        if not deterministic:
            delimiter |= pynini.cross("-", " dash ") | pynini.cross(
                "/", " slash "
            )
        ...
        next_alpha_or_num |= pynini.closure(
            ...
            + plurals._priority_union(
                pynini.accep(" "), pynutil.insert(" "), NEMO_SIGMA
            ).optimize()
            + alphas
        )
        ...
        serial_graph |= pynini.compose(
            NEMO_SIGMA + symbols + NEMO_SIGMA, num_graph + delimiter + num_graph
        )

        # exclude ordinal numbers from serial options
        serial_graph = pynini.compose(
            pynini.difference(
                NEMO_SIGMA, pynini.project(ordinal.graph, "input")
            ),
            serial_graph,
        ).optimize()
        ...
        serial_graph |= pynini.compose(
            graph_with_space, serial_graph.optimize()
        ).optimize()
        serial_graph = pynini.compose(
