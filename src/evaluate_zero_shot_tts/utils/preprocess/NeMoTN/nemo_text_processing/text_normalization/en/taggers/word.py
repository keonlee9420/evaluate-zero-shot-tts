...
class WordFst(GraphFst):
    ...
    def __init__(self, punctuation: GraphFst, deterministic: bool = True):
        super().__init__(
            name="word", kind="classify", deterministic=deterministic
        )
        ...
        symbols_to_exclude = (
            pynini.union("$", "€", "₩", "£", "¥", "#", "%") | NEMO_DIGIT
        ).optimize()
        graph = pynini.closure(
            pynini.difference(NEMO_NOT_SPACE, symbols_to_exclude), 1
        )
        graph = pynutil.add_weight(graph, MIN_NEG_WEIGHT) | default_graph

        # leave phones of format [HH AH0 L OW1] untouched
        phoneme_unit = pynini.closure(NEMO_ALPHA, 1) + pynini.closure(
            NEMO_DIGIT
        )
        phoneme = (
        ...
        ipa_phoneme_unit = pynini.string_file(
            get_abs_path("data/whitelist/ipa_symbols.tsv")
        )
        # word in ipa form
        ipa_phonemes = (
        ...
        ipa_phonemes = (
            ipa_phonemes
            + pynini.closure(delim + ipa_phonemes)
            + pynini.closure(delim, 0, 1)
        )
        ipa_phonemes = (
            pynini.accep(pynini.escape("["))
            + ipa_phonemes
        ...
        self.graph = plurals._priority_union(
            convert_space(phoneme.optimize()), graph, NEMO_SIGMA
        )
        self.fst = (
