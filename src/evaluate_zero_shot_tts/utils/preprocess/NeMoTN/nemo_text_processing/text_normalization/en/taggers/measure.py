...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        graph_unit |= pynini.compose(
            ...
            + (NEMO_ALPHA | TO_LOWER)
            + pynini.closure(NEMO_ALPHA | TO_LOWER),
            graph_unit,
        ).optimize()
        ...
        math_operations = pynini.string_file(
            get_abs_path("data/measure/math_operation.tsv")
        )
    ...
    def get_address_graph(self, cardinal):
        ...
        address_num = plurals._priority_union(
            address_num, cardinal.graph, NEMO_SIGMA
        )
        ...
        address_words = (
            ...
            + (
                ...
                | NEMO_UPPER + pynini.closure(NEMO_ALPHA, 1)
            )
            + NEMO_SPACE
            + pynini.closure(
                NEMO_UPPER + pynini.closure(NEMO_ALPHA) + NEMO_SPACE
            )
            + address_words
        )
