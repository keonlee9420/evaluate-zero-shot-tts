...
graph_teen = pynini.invert(
    pynini.string_file(get_abs_path("data/number/teen.tsv"))
).optimize()
graph_digit = pynini.invert(
    pynini.string_file(get_abs_path("data/number/digit.tsv"))
).optimize()
ties_graph = pynini.invert(
    pynini.string_file(get_abs_path("data/number/ty.tsv"))
).optimize()
...
def get_ties_graph(deterministic: bool = True):
    ...
    if deterministic:
        ...
        graph = (
            graph
            | (pynini.cross("0", "oh") | pynini.cross("0", "zero"))
            + insert_space
            + graph_digit
        )
...
def _get_year_graph(cardinal_graph, deterministic: bool = True):
    ...
    three_digit_year = (
        (NEMO_DIGIT @ cardinal_graph)
        + insert_space
        + (NEMO_DIGIT**2) @ cardinal_graph
    )
    year_with_suffix = (
...
def _get_financial_period_graph():
    ...
    q_ordinals = (
        h_ordinals | pynini.cross("3", "third") | pynini.cross("4", "fourth")
    )
...
class DateFst(GraphFst):
    ...
    def __init__(
        self, cardinal: GraphFst, deterministic: bool, lm: bool = False
    ):
        super().__init__(
            name="date", kind="classify", deterministic=deterministic
        )

        # january
        month_graph = pynini.string_file(
            get_abs_path("data/date/month_name.tsv")
        ).optimize()
        # January, JANUARY
        month_graph |= pynini.compose(
            TO_LOWER + pynini.closure(NEMO_CHAR), month_graph
        ) | pynini.compose(TO_LOWER ** (2, ...), month_graph)

        # jan
        month_abbr_graph = pynini.string_file(
            get_abs_path("data/date/month_abbr.tsv")
        ).optimize()
        # jan, Jan, JAN
        month_abbr_graph = (
            month_abbr_graph
            | pynini.compose(
                TO_LOWER + pynini.closure(NEMO_LOWER, 1), month_abbr_graph
            ).optimize()
        ...
        year_graph = _get_year_graph(
            cardinal_graph=cardinal_graph, deterministic=deterministic
        )
        ...
        month_graph = (
            pynutil.insert('month: "') + month_graph + pynutil.insert('"')
        )
        month_numbers_graph = (
            pynutil.insert('month: "')
            + month_numbers_labels
            + pynutil.insert('"')
        )
        ...
        two_digit_year = (
            pynutil.insert('year: "') + two_digit_year + pynutil.insert('"')
        )
        ...
        graph_mdy |= (
            ...
            + day_graph
            + pynini.closure(
                ((pynini.cross("-", " ") + NEMO_SIGMA) @ graph_year), 0, 1
            )
        )
        ...
        graph_dmy = (
            day_graph + delete_extra_space + month_graph + optional_graph_year
        )
        day_ex_month = (
            NEMO_DIGIT**2 - pynini.project(month_numbers_graph, "input")
        ) @ day_graph
        for x in ["-", "/", "."]:
            delete_sep = pynutil.delete(x)
            graph_dmy |= (
        ...
        if not deterministic or lm:
            ...
            for month in [
                x[0]
                for x in load_labels(get_abs_path("data/date/month_name.tsv"))
            ]:
                for day in [
                    x[0] for x in load_labels(get_abs_path("data/date/day.tsv"))
                ]:
                    ymd_to_mdy_curr = (
                        pynutil.insert(
                            'month: "' + month + '" day: "' + day + '" '
                    ...
                    # YY-MM-DD -> MM-DD-YY
                    ymd_to_dmy_curr = pynini.compose(
                        graph_ymd, ymd_to_dmy_curr
                    ).optimize()
                    ymd_to_dmy_graph = (
                        ymd_to_dmy_curr
                    ...
                    mdy_to_dmy_curr = pynini.compose(
                        graph_mdy, mdy_to_dmy_curr
                    ).optimize()
                    mdy_to_dmy_graph = (
                        mdy_to_dmy_curr
                        if mdy_to_dmy_graph is None
                        else pynini.union(
                            mdy_to_dmy_curr, mdy_to_dmy_graph
                        ).optimize()
                    ).optimize()
                    ...
                    md_to_dm_curr = pynini.compose(
                        m_sep_d, md_to_dm_curr
                    ).optimize()
