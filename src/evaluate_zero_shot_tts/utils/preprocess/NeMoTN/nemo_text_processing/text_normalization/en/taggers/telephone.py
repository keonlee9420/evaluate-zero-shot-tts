...
class TelephoneFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        number_length = (
            (NEMO_DIGIT + del_separator) | (NEMO_ALPHA + del_separator)
        ) ** 7
        ...
        graph = plurals._priority_union(
            country_code + number_part, number_part, NEMO_SIGMA
        ).optimize()
        graph = plurals._priority_union(
            country_code + number_part + extension, graph, NEMO_SIGMA
        ).optimize()
        graph = plurals._priority_union(
            number_part + extension, graph, NEMO_SIGMA
        ).optimize()
        ...
        ip_graph = (
            digit_to_str_graph
            + (pynini.cross(".", " dot ") + digit_to_str_graph) ** 3
        )
        graph |= (
            pynini.closure(
                pynutil.insert('country_code: "')
