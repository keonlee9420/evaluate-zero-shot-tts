...
class RangeFst(GraphFst):
    ...
    def __init__(
        ...
        super().__init__(
            name="range", kind="classify", deterministic=deterministic
        )
        ...
        date_year_four_digit = (
            NEMO_DIGIT**4 + pynini.closure(pynini.accep("s"), 0, 1)
        ) @ date
        date_year_two_digit = (
            NEMO_DIGIT**2 + pynini.closure(pynini.accep("s"), 0, 1)
        ) @ date
        year_to_year_graph = (
            date_year_four_digit
            + delete_space
