...
class WhiteListFst(GraphFst):
    ...
    def __init__(
        ...
        def _get_whitelist_graph(
            input_case, file, keep_punct_add_end: bool = False
        ):
        ...
        graph = _get_whitelist_graph(
            input_case, get_abs_path("data/whitelist/tts.tsv")
        )
        graph |= pynini.compose(
            pynini.difference(NEMO_SIGMA, pynini.accep("/")).optimize(),
            _get_whitelist_graph(
                input_case, get_abs_path("data/whitelist/symbol.tsv")
            ),
        ).optimize()
        ...
        if deterministic:
            ...
            graph |= _get_whitelist_graph(
                input_case,
                get_abs_path("data/whitelist/alternatives.tsv"),
        ...
        if not deterministic:
            ...
            units_graph = pynini.compose(
                NEMO_CHAR ** (3, ...),
                convert_space(graph_unit | graph_unit_plural),
            )
            graph |= units_graph
