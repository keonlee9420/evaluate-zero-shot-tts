...
class Normalizer:
    ...
    def __init__(
        ...
        if lang == "en":
            ...
            from nemo_text_processing.text_normalization.ru.taggers.tokenize_and_classify import (
                ClassifyFst,
            )
            from nemo_text_processing.text_normalization.ru.verbalizers.verbalize_final import (
                VerbalizeFinalFst,
            )
        elif lang == "de":
            ...
            from nemo_text_processing.text_normalization.de.verbalizers.verbalize_final import (
                VerbalizeFinalFst,
            )
        elif lang == "es":
            ...
            from nemo_text_processing.text_normalization.es.verbalizers.verbalize_final import (
                VerbalizeFinalFst,
            )
        elif lang == "fr":
            from .fr.taggers.tokenize_and_classify import ClassifyFst
            from .fr.verbalizers.verbalize_final import VerbalizeFinalFst
        elif lang == "sv":
            from nemo_text_processing.text_normalization.sv.taggers.tokenize_and_classify import (
                ClassifyFst,
            )
            from nemo_text_processing.text_normalization.sv.verbalizers.verbalize_final import (
                VerbalizeFinalFst,
            )
        elif lang == "hu":
            ...
            from nemo_text_processing.text_normalization.hu.verbalizers.verbalize_final import (
                VerbalizeFinalFst,
            )
        elif lang == "zh":
            ...
            from nemo_text_processing.text_normalization.zh.verbalizers.verbalize_final import (
                VerbalizeFinalFst,
            )
        elif lang == "ar":
            ...
            from nemo_text_processing.text_normalization.ar.verbalizers.verbalize_final import (
                VerbalizeFinalFst,
            )
        elif lang == "it":
            from .it.taggers.tokenize_and_classify import ClassifyFst
            from .it.verbalizers.verbalize_final import VerbalizeFinalFst
        else:
            raise NotImplementedError(
                f"Language {lang} has not been supported yet."
            )
    ...
    def normalize_list(
        ...
        **kwargs,
        ...
        def _process_batch(
            batch, verbose, punct_pre_process, punct_post_process, **kwargs
        ):
            """
    ...
    def _estimate_number_of_permutations_in_nested_dict(
        ...
        for k, inner in token_group.items():
            if isinstance(inner, dict):
                num_perms *= (
                    self._estimate_number_of_permutations_in_nested_dict(inner)
                )
    ...
    def _split_tokens_to_reduce_number_of_permutations(
        ...
        for i, token_group in enumerate(tokens):
            ...
            if (
                n * current_number_of_permutations
                > self.max_number_of_permutations_per_split
            ):
    ...
    def normalize(
        ...
        if punct_post_process:
            ...
            output = post_process_punct(
                input=original_text, normalized_text=output
            )
        return output
    ...
    def split_text_into_sentences(
        self, text: str, additional_split_symbols: str = ""
    ) -> List[str]:
        """
        ...
        matches = re.findall(
            rf"[a-z{lower_case_unicode}]\.\s[a-z{lower_case_unicode}]\.", text
        )
    ...
    def post_process(self, normalized_text: "pynini.FstLike") -> str:
        ...
        if self.post_processor is not None:
            normalized_text = top_rewrite(
                normalized_text, self.post_processor.fst
            )
        return normalized_text
...
def parse_args():
    ...
    input.add_argument(
        "--text", dest="input_string", help="input string", type=str
    )
    input.add_argument(
        "--input_file",
        dest="input_file",
    ...
    parser.add_argument(
        "--output_file", dest="output_file", help="Output file path", type=str
    )
    parser.add_argument(
        "--language",
        help="language",
    ...
    parser.add_argument(
        "--verbose", help="print info for debugging", action="store_true"
    )
    parser.add_argument(
        "--punct_post_process",
    ...
    parser.add_argument(
        "--overwrite_cache",
        help="Add this flag to re-create .far grammar files",
        action="store_true",
    )
    parser.add_argument(
        "--whitelist",
    ...
    parser.add_argument(
        ...
        default=200,
        type=int,
        help="Number of examples for each process",
    )
    parser.add_argument(
        "--max_number_of_permutations_per_split",
        default=729,
