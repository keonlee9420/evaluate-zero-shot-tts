...
class CardinalFst(GraphFst):
    def __init__(self):
        ...
        graph_tens = (ties + graph_digits) | (
            pynini.cross(pynini.accep("零"), "0") + graph_digits
        )
        ...
        graph_millions = (
            pynutil.add_weight(graph_millions_simple, -1.0)
            | graph_millions_complex
            | pynutil.insert("0000000")
        )

        # grammar for ten millions 千万
        graph_ten_millions_simple = (
            graph_thousands_complex + closure_ten_thousands
        )
        ...
        graph_ten_millions = (
            pynutil.add_weight(graph_ten_millions_simple, -1.0)
            | graph_ten_millions_complex
        )
        ...
        graph_billions = (
            pynutil.add_weight(graph_billions_simple, -1.0)
            | graph_billions_complex
            | pynutil.insert("0000000000")
        )

        # grammar for ten billions 百亿
        graph_ten_billions_simple = (
            graph_hundreds_complex + closure_hundred_millions
        )
        ...
        graph_hundred_billions_simple = (
            graph_thousands_complex + closure_hundred_millions
        )
        ...
        graph_hundred_billions = (
            pynutil.add_weight(graph_hundred_billions_simple, -1.0)
            | graph_hundred_billions_complex
        )
        ...
        rest_of_cardinal = pynini.closure(NEMO_DIGIT) | pynini.closure(
            NEMO_SIGMA
        )

        # output for cardinal grammar without leading zero
        clean_cardinal = (
            delete_leading_zeros + stop_at_non_zero + rest_of_cardinal
        )
        ...
        graph_just_cardinals = (
            graph_just_cardinals @ clean_just_cardinal
        )  # output for other grammars
        ...
        optional_minus_graph = (
            pynini.closure(
                pynutil.insert("negative: ") + pynini.cross("负", '"-"')
            )
        ) | (
            pynini.closure(
                pynutil.insert("negative: ") + pynini.cross("負", '"-"')
            )
        )
        final_graph = (
            optional_minus_graph
