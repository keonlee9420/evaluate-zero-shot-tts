...
class TimeFst(GraphFst):
    ...
    def __init__(self, input_case: str = INPUT_LOWER_CASED):
        ...
        if input_case == INPUT_CASED:
            suffix_graph |= pynini.string_file(
                get_abs_path("data/time/time_suffix_cased.tsv")
            )
            time_zone_graph |= pynini.invert(
                pynini.string_file(
                    get_abs_path("data/time/time_zone_cased.tsv")
                )
            )

        to_hour_graph = pynini.string_file(
            get_abs_path("data/time/to_hour.tsv")
        )
        minute_to_graph = pynini.string_file(
            get_abs_path("data/time/minute_to.tsv")
        )

        # only used for < 1000 thousand -> 0 weight
        cardinal = pynutil.add_weight(
            CardinalFst(input_case=input_case).graph_no_exception, weight=-0.7
        )
        ...
        graph_m_to_h_suffix_time = (
            pynutil.insert('minutes: "')
            + (
                (graph_minute_single | graph_minute_double).optimize()
                @ minute_to_graph
            )
            + pynutil.insert('"')
            + pynini.closure(
                delete_space
                + pynutil.delete(
                    pynini.union("min", "mins", "minute", "minutes")
                ),
                0,
                1,
            )
