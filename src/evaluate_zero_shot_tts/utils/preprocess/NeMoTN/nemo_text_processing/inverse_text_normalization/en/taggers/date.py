...
def _get_range_graph(input_case: str):
    ...
    graph = (
        ...
        + delete_space
        + pynini.cross("hundreds", "00s")
    )
    graph |= (
        pynini.cross("two", "2")
        + delete_space
        + pynini.cross("thousands", "000s")
    )
    graph |= (
        (graph_ties | graph_teen)
        + delete_space
        + (
            ...
            + (pynini.cross("ies", "y") | pynutil.delete("s"))
        )
...
def _get_year_graph(input_case: str):
    ...
    def _get_thousands_graph():
        ...
        graph = (
            ...
            + (
                graph_teen
                | graph_ties
                | (optional_end + pynutil.insert("0") + graph_digit)
            )
        )
    ...
    graph_ad_bc = (
        delete_space
        + pynini.string_file(get_abs_path("data/year_suffix.tsv")).invert()
    )
...
class DateFst(GraphFst):
    ...
    def __init__(self, ordinal: GraphFst, input_case: str):
        ...
        day_graph = (
            pynutil.insert('day: "')
            + pynutil.add_weight(ordinal_graph, -0.7)
            + pynutil.insert('"')
        )
        graph_year = (
            delete_extra_space
        ...
        financial_period_graph = pynini.string_file(
            get_abs_path("data/date_period.tsv")
        ).invert()
        period_fy = (
            pynutil.insert('text: "')
        ...
        graph_year = (
            pynutil.insert('year: "')
            + (year_graph | _get_range_graph(input_case=input_case))
            + pynutil.insert('"')
        )
