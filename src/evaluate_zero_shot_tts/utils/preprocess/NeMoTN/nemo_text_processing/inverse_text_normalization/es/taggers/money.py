...
class MoneyFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, decimal: GraphFst):
        ...
        unit_singular = pynini.string_file(
            get_abs_path("data/money/currency_major_singular.tsv")
        )
        unit_singular = pynini.invert(unit_singular)
        unit_plural = pynini.string_file(
            get_abs_path("data/money/currency_major_plural.tsv")
        )
        unit_plural = pynini.invert(unit_plural)

        unit_minor_singular = pynini.string_file(
        ...
        graph_unit_minor_singular = (
            pynutil.insert('currency: "')
            + convert_space(unit_minor_singular)
            + pynutil.insert('"')
        )
        graph_unit_minor_plural = (
            pynutil.insert('currency: "')
            + convert_space(unit_minor_plural)
            + pynutil.insert('"')
        )
        ...
        optional_cents_standalone = pynini.closure(
            ...
            + cents_standalone
            + pynutil.delete(
                pynini.union(unit_minor_singular, unit_minor_plural)
            ),
            0,
            1,
        )
        ...
        graph_decimal = (
            graph_decimal_final + delete_extra_space + graph_unit_plural
        ) | cents_only_graph
        graph_decimal |= (
            graph_decimal_final
            + pynutil.delete(" de")
