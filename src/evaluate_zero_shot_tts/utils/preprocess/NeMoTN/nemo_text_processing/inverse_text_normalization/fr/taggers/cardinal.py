...
def rewrite(cardinal: "pynini.FstLike") -> "pynini.FstLike":
    ...
    filter = pynini.cdrewrite(
        pynini.cross(targets_for_filtering, "#"), "", "", NEMO_SIGMA
    ...
    replace_left = pynini.cdrewrite(
        pynini.cross(" ", "-"), "", targets, NEMO_SIGMA
    )

    replace_right = pynini.cdrewrite(
        pynini.cross(" ", "-"), targets, "", NEMO_SIGMA
    )
...
class CardinalFst(GraphFst):
    ...
    def __init__(self):
        ...
        graph_tens_component = pynini.union(
            graph_tens_component, graph_teens, graph_ties_unique
        )
        ...
        graph_hundreds_component_plural = (
            graph_digit_no_one + delete_hyphen + graph_cent_plural
        )
