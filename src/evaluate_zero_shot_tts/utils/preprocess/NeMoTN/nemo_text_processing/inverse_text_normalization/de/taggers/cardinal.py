...
class CardinalFst(GraphFst):
    ...
    def __init__(
        self, tn_cardinal_tagger: GraphFst, deterministic: bool = True
    ):
        super().__init__(
        ...
        graph = (
            (tn_cardinal_tagger.graph @ optional_delete_space)
            .invert()
            .optimize()
        )
        ...
        self.graph_ties = (
            (tn_cardinal_tagger.two_digit_non_zero @ optional_delete_space)
            .invert()
            .optimize()
        )
        ...
        self.digit = (
            pynini.arcmap(tn_cardinal_tagger.digit, map_type="rmweight")
            .invert()
            .optimize()
        )
