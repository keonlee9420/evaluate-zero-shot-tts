...
class DateFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        day_graph = (
            pynutil.insert('day: "') + cardinal_graph + pynutil.insert('"')
        )
        # day_suffix = pynini.union("ngày", "mùng")
        # optional_day = pynini.closure(day_suffix + delete_space, 0, 1)

        graph_month = (
            pynutil.delete("tháng") + delete_space + month_graph_exception
        )
        graph_year = (
            delete_extra_space
            + pynutil.delete("năm")
        ...
        graph_my = (
            pynutil.delete("tháng") + delete_space + month_graph + graph_year
        )
        graph_dmy = (
            day_graph
            + delete_space
