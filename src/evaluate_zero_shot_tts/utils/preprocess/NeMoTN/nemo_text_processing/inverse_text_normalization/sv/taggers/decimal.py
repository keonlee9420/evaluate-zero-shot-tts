...
class DecimalFst(GraphFst):
    ...
    def __init__(
        self, itn_cardinal_tagger: GraphFst, tn_decimal_tagger: GraphFst
    ):
        super().__init__(name="decimal", kind="classify")

        self.graph = tn_decimal_tagger.graph_itn
        self.graph = self.graph @ pynini.cdrewrite(
        ...
        graph_integer = (
            pynutil.insert('integer_part: "') + hundreds + pynutil.insert('"')
        )
        self.graph_integer = graph_integer
        final_graph_wo_sign = (
            graph_integer + delete_point + pynini.accep(" ") + graph_fractional
        )
