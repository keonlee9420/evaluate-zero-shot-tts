...
class OrdinalFst(GraphFst):
    ...
    def __init__(self, tn_ordinal: GraphFst):
        ...
        graph = (
            pynini.arcmap(tn_ordinal.bare_ordinals, map_type="rmweight")
            .invert()
            .optimize()
        )
        ...
        forsta_andra = pynini.project(
            pynini.union("1", "2") @ tn_ordinal.bare_ordinals, "output"
        )
        graph = (
            (pynini.project(graph, "input") - forsta_andra.arcsort()) @ graph
        ) + pynutil.insert(".")
