...
class InverseNormalizer(Normalizer):
    ...
    def inverse_normalize_list(
        self, texts: List[str], verbose=False
    ) -> List[str]:
        """
...
def parse_args():
    ...
    input.add_argument(
        "--input_file", dest="input_file", help="input file path", type=str
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "e.g., inverse_normalization/en/data/whitelist.tsv",
        default=None,
        type=str,
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    print(
        f"Time to generate graph: {round(perf_counter() - start_time, 2)} sec"
    )

    if args.input_string:
        print(
            inverse_normalizer.inverse_normalize(
                args.input_string, verbose=args.verbose
            )
        )
        ...
        prediction = inverse_normalizer.inverse_normalize_list(
