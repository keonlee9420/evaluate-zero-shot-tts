...
class FractionFst(GraphFst):
    ...
    def __init__(self, tn_cardinal: GraphFst):
        ...
        denominator_plural = pynini.union(
            "أخماس",
            "أرباع",
            "أثلاث",
            "أسداس",
        ...
        numerator_three_to_ten = pynini.union(
            "خمسة", "سبعة", "عشرة", "ثلاثة", "أربعة", "ستة", "ثمانية", "تسعة"
        )
        ...
        graph_dual = (
            pynini.string_file(get_abs_path("data/number/fraction_dual.tsv"))
            .invert()
            .optimize()
        )
        graph_plural = (
            pynini.string_file(get_abs_path("data/number/fraction_plural.tsv"))
            .invert()
            .optimize()
        )
        ...
        graph_denominator_singular = (
            ...
            + denominator_singular @ graph_ones
            + pynutil.insert('"')
        )
        ...
        fraction = (
            ...
            | numerator_three_to_ten
            | numerator_two
            | pynutil.add_weight(numerators, 0.001)
