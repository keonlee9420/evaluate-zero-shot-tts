    ...
    def __init__(
        ...
        if not overwrite_cache and far_file and os.path.exists(far_file):
            ...
            measure_graph = MeasureFst(
                cardinal=cardinal, decimal=decimal, fraction=fraction
            ).fst
            ...
            en_ordinal = EnOrdinalFst(
                cardinal=en_cardinal, input_case=input_case
            )
            ...
            en_measure_graph = EnMeasureFst(
                cardinal=en_cardinal, decimal=en_decimal, input_case=input_case
            ).fst
            en_date_graph = EnDateFst(
                ordinal=en_ordinal, input_case=input_case
            ).fst
            ...
            en_whitelist_graph = EnWhiteListFst(
                input_file=en_whitelist, input_case=input_case
            ).fst
            ...
            en_punct = (
                pynutil.insert("tokens { ")
                + pynutil.add_weight(en_punct_graph, weight=1.3)
                + pynutil.insert(" }")
            )
            token = (
