...
class TestMeasure:
    inverse_normalizer = (
        InverseNormalizer(lang="ar", cache_dir=CACHE_DIR, overwrite_cache=False)
        if PYNINI_AVAILABLE
        else None
    )
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ar/data_inverse_text_normalization/test_cases_measure.txt"
        )
    )
    @pytest.mark.skipif(
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ar/data_text_normalization/test_cases_measure.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
