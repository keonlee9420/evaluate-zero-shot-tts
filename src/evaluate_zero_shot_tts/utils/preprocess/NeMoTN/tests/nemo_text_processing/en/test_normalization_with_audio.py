...
class TestNormalizeWithAudio:
    ...
    @parameterized.expand(
        get_test_cases_multiple(
            "en/data_text_normalization/test_cases_normalize_with_audio.txt"
        )
    )
    ...
    def test_norm(self, test_input, expected):
        if self.normalizer_with_audio_en:
            pred = self.normalizer_with_audio_en.normalize(
                test_input, n_tagged=50, punct_post_process=True
            )
