...
class TestWord:
    inverse_normalizer_es = InverseNormalizer(
        lang="es", cache_dir=CACHE_DIR, overwrite_cache=False
    )

    @parameterized.expand(
        parse_test_case_file(
            "es/data_inverse_text_normalization/test_cases_word.txt"
        )
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_denorm_es(self, test_input, expected):
        pred = self.inverse_normalizer_es.inverse_normalize(
            test_input, verbose=False
        )
        assert pred == expected

    normalizer_es = Normalizer(
        input_case="cased",
        lang="es",
        cache_dir=CACHE_DIR,
    ...
    @parameterized.expand(
        parse_test_case_file("es/data_text_normalization/test_cases_word.txt")
    )
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
