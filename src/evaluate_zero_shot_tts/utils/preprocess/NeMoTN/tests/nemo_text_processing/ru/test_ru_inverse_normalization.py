...
class TestRuInverseNormalize:
    normalizer = (
        InverseNormalizer(lang="ru", cache_dir=CACHE_DIR)
        if PYNINI_AVAILABLE
        else None
    )
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_cardinal.txt"
        )
    )
    @pytest.mark.skipif(
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_ordinal.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_decimal.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_electronic.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_date.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_measure.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_money.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_time.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_whitelist.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
    ...
    @parameterized.expand(
        parse_test_case_file(
            "ru/data_inverse_text_normalization/test_cases_word.txt"
        )
    )
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
