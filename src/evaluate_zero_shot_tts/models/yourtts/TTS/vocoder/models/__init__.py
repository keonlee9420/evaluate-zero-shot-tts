...
def setup_model(config: Coqpit):
    ...
    if "discriminator_model" in config and "generator_model" in config:
        MyModel = importlib.import_module("yourtts.TTS.vocoder.models.gan")
        MyModel = getattr(MyModel, "GAN")
    else:
        MyModel = importlib.import_module("yourtts.TTS.vocoder.models." + config.model.lower())
...
def setup_generator(c):
    ...
    MyModel = importlib.import_module("yourtts.TTS.vocoder.models." + c.generator_model.lower())
...
def setup_discriminator(c):
    ...
    if "parallel_wavegan" in c.discriminator_model:
        MyModel = importlib.import_module("yourtts.TTS.vocoder.models.parallel_wavegan_discriminator")
    else:
        MyModel = importlib.import_module("yourtts.TTS.vocoder.models." + c.discriminator_model.lower())
